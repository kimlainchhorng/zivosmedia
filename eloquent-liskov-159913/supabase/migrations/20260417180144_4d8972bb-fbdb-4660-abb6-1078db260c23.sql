...
CREATE INDEX idx_live_pair_sessions_token ON public.live_pair_sessions(token);
CREATE INDEX idx_live_pair_sessions_owner ON public.live_pair_sessions(store_owner_id);

ALTER TABLE public.live_pair_sessions ENABLE ROW LEVEL SECURITY;

-- Owner (desktop) can read their own pending/recent sessions in realtime
CREATE POLICY "Owners view their own pair sessions"
  ON public.live_pair_sessions
  FOR SELECT
  USING (auth.uid() = store_owner_id);

-- No direct INSERT/UPDATE/DELETE from clients — all via SECURITY DEFINER fns below
-- (no other policies => denied)
...
-- ============================================================
-- create_live_pair_session: desktop owner creates a QR session
-- ============================================================
...
-- ============================================================
-- get_live_pair_session: phone (anon) fetches safe session info
-- ============================================================
...
-- ============================================================
-- confirm_live_pair_session: phone confirms pairing
-- ============================================================
...
-- ============================================================
-- cancel_live_pair_session: phone declines pairing
-- ============================================================
