 ...
 * ----------------------------
 * Server-gated owner/admin dealership review moderation.
 */
import { createClient, serve } from "../_shared/deps.ts";
import { withSecurity } from "../_shared/withSecurity.ts";

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{12}$/i;
const ACTIONS = new Set(["respond", "set_visible", "delete"]);
...
serve(withSecurity("car-dealership-review-manage", async (req, ctx) => {
  ...
  if (action === "delete") {
    const { error } = await admin.from("car_dealership_reviews").delete().eq("id", reviewId).eq("store_id", storeId);
    if (error) {
      console.error("[car-dealership-review-manage:delete]", error.message);
  ...
  return updateReview(admin, reviewId, storeId, {
    ...
    owner_responded_at: response ? new Date().toISOString() : null,
  }, json);
...
async function updateReview(admin: any, reviewId: string | null, storeId: string, patch: Record<string, unknown>, json: (body: unknown, status?: number) => Response) {
  const { data, error } = await admin
    ...
    .single();
  if (error) {
    console.error("[car-dealership-review-manage:update]", error.message);
...
async function canManageStore(admin: any, userId: string, storeId: string): Promise<boolean> {
  const { data: store, error: storeError } = await admin
    ...
    .maybeSingle();
  if (storeError) {
    console.error("[car-dealership-review-manage:store]", storeError.message);
    return false;
  }
  ...
  if (roleError) {
    console.error("[car-dealership-review-manage:role]", roleError.message);
    return false;
  }
...
async function getReviewStoreId(admin: any, reviewId: string | null): Promise<string | null> {
  ...
  const { data, error } = await admin.from("car_dealership_reviews").select("store_id").eq("id", reviewId).maybeSingle();
  if (error) {
    console.error("[car-dealership-review-manage:review-store]", error.message);
    return null;
  }
