...
const Email = ({
  ...
  <Html lang="en">
    <Head />
    <Preview>Order confirmed at {restaurantName}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={badge}>ZIVO Eats</Section>
        <Heading style={h1}>Your order is in 🎉</Heading>
        <Text style={text}>
          We received your payment and {restaurantName} is preparing your meal. Use the tracking code below to follow it from kitchen to door.
        </Text>

        <Section style={card}>
          <Text style={kvLabel}>Tracking</Text>
          <Text style={kvValueStrong}>{trackingCode || '—'}</Text>
          <Hr style={hr} />
          <Text style={kvLabel}>Restaurant</Text>
          <Text style={kvValue}>{restaurantName}</Text>
        ...
        {restaurantPhone ? (
          <Text style={small}>Need to reach the restaurant? <strong>{restaurantPhone}</strong></Text>
        ) : null}

        <Hr style={hr} />
        <Text style={footer}>You can cancel from the order tracking page until the driver picks up. Refunds go back to the same payment method.</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  component: Email,
  subject: (p: Props) => `Order confirmed${p.trackingCode ? ` — ${p.trackingCode}` : ''} · ${p.restaurantName ?? 'ZIVO Eats'}`,
  displayName: 'Eats order confirmed',
  previewData: {
    restaurantName: 'Phnom Penh Noodle House',
    restaurantPhone: '+855 12 345 678',
    trackingCode: 'EAT-9472',
    deliveryAddress: '123 Riverside, Phnom Penh',
    totalAmount: '$18.50',
    paymentMethod: 'Card',
    trackUrl: 'https://zivosmedia.com/eats/track/abc',
  },
