/**
 * Customer Eats cancellation. Database claims serialize cancellation with
 * settlement/dispatch; every provider refund is bound to exact captured
 * evidence before the request can report success.
 */
...
async function reversePayoutIfNeeded(
  ...
  try {
    let { data, error } = await admin.rpc("claim_eats_payout_reversal", {
      p_order_id: orderId,
    });
    let claim = rpcObject(data);
    if (error || !claim?.ok)
      throw new Error(
        error?.message || claim?.code || "payout_reversal_claim_pending",
      );
    ...
    if (claim.transfer_recovery_required === true) {
      ...
      ({ data, error } = await admin.rpc("claim_eats_payout_reversal", {
        p_order_id: orderId,
      }));
      claim = rpcObject(data);
      if (error || !claim?.ok)
        throw new Error(
          error?.message || claim?.code || "payout_reversal_claim_pending",
        );
    ...
    const reversal = await stripe.transfers.createReversal(
      String(claim.stripe_transfer_id),
      {
        ...
        metadata: { order_id: orderId, reason: "eats_cancel" },
      },
      { idempotencyKey: String(claim.idempotency_key) },
    );
    ...
    async (req, ctx) => {
      ...
      try {
        ...
        const { data: order, error: orderError } = await admin
          .from("food_orders")
          .select(
            "id, customer_id, status, payment_type, payment_status, payment_provider, total_amount, driver_id, tracking_code",
          )
          .eq("id", orderId)
          .maybeSingle();
        if (orderError)
          return json({ error: "order_lookup_pending", retryable: true }, 503);
        ...
        if (["delivered", "completed"].includes(status))
          return json(
            { error: "already_inactive", current_status: status },
            409,
          );

        const driverAssigned = Boolean((order as any).driver_id);
        const eligible =
          !driverAssigned &&
          !["in_transit", "out_for_delivery", "picked_up"].includes(status);
        const previewCents = eligible
          ? Math.round(Number((order as any).total_amount ?? 0) * 100)
          : 0;
        if (body.preview === true) {
          return json({
            ...
            reason_label: eligible
              ? "Full refund"
              : driverAssigned
                ? "Driver already on the way — refund not available"
                : "Order already in transit",
          });
        }

        const walletDebit = await hasOriginalWalletDebit(
          admin,
          user.id,
          orderId,
        );
        if (walletDebit.lookupFailed)
          return json(
            { error: "wallet_payment_verification_pending", retryable: true },
            503,
          );
        const isWallet =
          walletDebit.found ||
          (order as any).payment_type === "wallet" ||
          (order as any).payment_provider === "wallet";
        let paymentStatus = String((order as any).payment_status ?? "unpaid");
        let actualRefundCents = 0;
        let provider = String(
          (order as any).payment_provider ?? (order as any).payment_type ?? "",
        );
        let providerRefundId: string | null = null;
        let remainingRefunds = 0;
        let shouldReversePayout = status === "refunded";

        if (status === "refunded" || paymentStatus === "refunded") {
          await cascadeDurably(admin, orderId);
          await reversePayoutIfNeeded(admin, orderId).catch((error) => {
            throw new Error(
              `payout_reversal_pending:${error instanceof Error ? error.message : String(error)}`,
            );
          });
          return json({
            ok: true,
        ...
        if (isWallet) {
          const claimed = await admin.rpc("claim_eats_wallet_cancellation", {
            p_order_id: orderId,
            p_customer_id: user.id,
            p_reason: reason,
          });
          const claim = rpcObject(claimed.data);
          if (claimed.error || !claim?.ok)
            return json(
              {
                error: claim?.code ?? "wallet_cancellation_pending",
                retryable: true,
              },
              503,
            );
          ...
          if (
            ...
            claim.wallet_refund_required !== true
          ) {
            return json(
              {
          ...
          if (claim.wallet_refund_required === true) {
            const refunded = await admin.rpc("process_eats_wallet_refund", {
              p_user_id: user.id,
              p_order_id: orderId,
              p_description: `Eats order ${(order as any).tracking_code ?? orderId} — cancellation refund`,
            });
            const result = rpcObject(refunded.data);
            if (refunded.error || !result || Number(result.refund_cents) <= 0) {
              return json(
                { error: "wallet_refund_pending", retryable: true },
                503,
              );
            }
            actualRefundCents = Number(result.refund_cents);
            paymentStatus = "refunded";
            shouldReversePayout = true;
          } else {
            paymentStatus = String(claim.payment_status ?? paymentStatus);
          }
          provider = "wallet";
        } else {
          const claimed = await admin.rpc("claim_eats_order_cancellation", {
            p_order_id: orderId,
            p_customer_id: user.id,
          });
          const claim = rpcObject(claimed.data);
        ...
        if (shouldReversePayout) {
          try {
            ...
            console.error("[cancel-eats-order] payout reversal pending", error);
            return json(
              {
        ...
        cascadeCancellationToDriver(admin, orderId, "delivery").catch((error) =>
          console.warn("[cancel-eats-order] driver push skipped", error),
        );
        if (actualRefundCents > 0) {
          ...
          notifyEatsRefundIssued(
            ...
            console.warn("[cancel-eats-order] refund notice skipped", error),
          );
        }
        ...
        console.error("[cancel-eats-order]", message);
        return new Response(
          JSON.stringify({
            error: "cancellation_recovery_pending",
            retryable: true,
          }),
          {
            status: 503,
...
async function hasOriginalWalletDebit(
  admin: any,
  userId: string,
  orderId: string,
): Promise<{ found: boolean; lookupFailed: boolean }> {
  ...
  if (error) {
    console.error(
      "[cancel-eats-order] wallet debit lookup failed",
      error.message,
    );
