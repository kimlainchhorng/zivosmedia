...
const MISSING_USER_BLOCKLIST_RPC = "PGRST202";
const MISSING_USER_BLOCKLIST_TABLE = "PGRST205";
...
serve(
  withSecurity(
    ...
    async (req, ctx) => {
      ...
      const usedSchemaCompatibility = await requireAuthorizationAccountActive(
        service,
        auth.userId,
      );
      if (usedSchemaCompatibility) {
        ctx.log.warn("user_blocklist_schema_compatibility", {
          missingRpcCode: MISSING_USER_BLOCKLIST_RPC,
        });
      }
      ...
      if (!isValidAppKey(body.app_key)) {
        await audit({
          eventType: "auth_code.issue.rejected",
          appKey: null,
          userId: auth.userId,
          success: false,
      ...
      const nonce =
        typeof body.nonce === "string" ? body.nonce.slice(0, 256) : null;
      ...
      if (insertError) {
        ctx.log.error("auth_code_insert_failed", {
          error: insertError.message,
        });
        await audit({
          eventType: "auth_code.issue.failed",
          appKey: body.app_key,
          userId: auth.userId,
          success: false,
...
async function requireAuthorizationAccountActive(
  service: ReturnType<typeof serviceClient>,
  userId: string,
): Promise<boolean> {
  const { data: isBlocked, error: blocklistError } = await service.rpc(
    "is_user_blocked",
    {
      _user_id: userId,
    },
  ...
  // The main project predates the optional custom blocklist migration. Keep
  // only the exact missing-RPC drift compatible. If the table has landed ahead
  // of its RPC, enforce active rows directly; every other lookup error fails
  // closed. Supabase Auth remains the authoritative suspension fallback.
  if (blocklistError && blocklistError.code !== MISSING_USER_BLOCKLIST_RPC) {
    throw new Error("User suspension check unavailable");
  }
  let customBlockActive = isBlocked === true;
  const usedSchemaCompatibility =
    blocklistError?.code === MISSING_USER_BLOCKLIST_RPC;

  if (usedSchemaCompatibility) {
    const { data: blocklistRow, error: blocklistTableError } = await service
      .from("user_blocklist")
    ...
    if (blocklistRow) {
      const expiresAt = blocklistRow.expires_at;
      if (expiresAt === null) {
        customBlockActive = true;
      } else {
        const expiresAtMs = Date.parse(expiresAt);
        if (!Number.isFinite(expiresAtMs)) {
          throw new Error("User suspension status unavailable");
        }
        customBlockActive = expiresAtMs > Date.now();
      }
    }
  }

  if (customBlockActive) {
  ...
  if (userError || !userData.user) {
    throw new HttpError(401, "Could not verify the authenticated user", {
      code: "invalid_session",
    });
  }

  const bannedUntil = userData.user.banned_until;
  if (!bannedUntil) return usedSchemaCompatibility;

  const bannedUntilMs = Date.parse(bannedUntil);
  if (!Number.isFinite(bannedUntilMs)) {
    throw new Error("User suspension status unavailable");
  }
  if (bannedUntilMs > Date.now()) {
    throw new HttpError(403, "Account suspended", { code: "user_blocked" });
  }
  return usedSchemaCompatibility;
}
...
async function audit(input: {
  ...
  await serviceClient()
    .from("zivosmedia_auth_audit_logs")
    .insert({
