 ...
 *
 * Auth model: guest-callable only with a short-lived, booking-bound `tip`
 * capability; an authenticated customer may instead use account ownership.
 * Customer visits /booking/:id after their visit, taps "Leave a tip", picks
 * an amount, and we charge the card saved during the deposit Checkout
 ...
 *
 * Idempotent:
 *   - A successful/processing PaymentIntent is set before returning, so a
 *     retry short-circuits at the "tip already submitted" check.
 *   - The Stripe key includes booking, amount, and the previous failure time:
 *     rapid double-taps converge, while a declined attempt can be retried.
 *
...
import {
  authorizeSalonBookingAction,
  cleanSalonBookingCapability,
  cleanSalonBookingId,
} from "../_shared/salonBookingAccess.ts";
  ...
  withSecurity(
    ...
    async (req, ctx) => {
      ...
      try {
        ...
        const bookingId = cleanSalonBookingId(body.booking_id);
        const accessToken = cleanSalonBookingCapability(body.access_token);
        const tipCents = Math.floor(Number(body.tip_cents ?? 0));
        if (!bookingId) {
          return new Response(
            JSON.stringify({ error: "Invalid booking request" }),
            {
              status: 400,
              headers: { ...cors, "Content-Type": "application/json" },
            },
        ...
        if (!Number.isFinite(tipCents) || tipCents <= 0) {
          return new Response(
            JSON.stringify({ error: "tip_cents must be a positive integer" }),
            {
              status: 400,
              headers: { ...cors, "Content-Type": "application/json" },
            },
        ...
        if (tipCents > MAX_TIP_CENTS) {
          return new Response(
            JSON.stringify({
              error: `Tip is capped at $${(MAX_TIP_CENTS / 100).toFixed(0)}. Please pay any excess in person.`,
            }),
            {
              status: 400,
              headers: { ...cors, "Content-Type": "application/json" },
        ...
        if (!authorized) {
          return new Response(
            JSON.stringify({ error: "Booking access denied or expired" }),
            {
              status: 403,
              headers: { ...cors, "Content-Type": "application/json" },
            },
        ...
        if (b.status !== "completed" && b.status !== "confirmed") {
          return new Response(
            JSON.stringify({ error: `Can't tip on a ${b.status} booking.` }),
            {
              status: 409,
              headers: { ...cors, "Content-Type": "application/json" },
            },
        ...
        if (
          (b.tip_cents ?? 0) > 0 ||
          !!b.tip_charged_at ||
          (!!b.tip_stripe_payment_intent_id && !b.tip_charge_failed_at)
        ) {
          return new Response(
            JSON.stringify({
        ...
        if (!b.stripe_customer_id || !b.stripe_payment_method_id) {
          return new Response(
            JSON.stringify({
              error: "No card on file for this booking — please tip in person.",
            }),
            {
              status: 409,
              headers: { ...cors, "Content-Type": "application/json" },
        ...
        const billCents =
          Number(b.price_cents ?? 0) + Number(b.addons_total_cents ?? 0);
        ...
        if (!tipsEnabled) {
          return new Response(
            JSON.stringify({
              error: "This salon isn't accepting online tips right now.",
            }),
            {
              status: 409,
              headers: { ...cors, "Content-Type": "application/json" },
        ...
        if (!accountId || stripeStatus !== "active") {
          return new Response(
            JSON.stringify({
              error: "This salon's Stripe account isn't active.",
            }),
            {
              status: 409,
              headers: { ...cors, "Content-Type": "application/json" },
        ...
        try {
          const pi = await stripe.paymentIntents.create(
            ...
            },
            {
              idempotencyKey: [
                ...
                b.id,
                tipCents,
                b.tip_charge_failed_at ?? "initial",
              ].join(":"),
            },
          );
          ...
          if (pi.status === "succeeded") {
            updatePatch.tip_cents =
              (Number(b.tip_cents ?? 0) || 0) +
              (pi.amount_received || pi.amount || tipCents);
          ...
          await supabase
            ...
            .eq("id", b.id);
          console.warn("[charge-salon-tip] charge failed", {
            booking: b.id,
            reason,
          });
