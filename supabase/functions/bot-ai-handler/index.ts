 ...
 *
 * On every incoming update we call AI model APIs (DeepSeek first, Claude fallback)
...
function providerOrder(provider: AiProvider | "auto") {
  if (provider === "claude") return ["claude", "deepseek"] as const;
  if (provider === "deepseek") return ["deepseek", "claude"] as const;
...
Deno.serve(withSecurity("bot-ai-handler", async (req, ctx) => {
  ...
  try {
    ...
    if (!deepseekApiKey && !anthropicApiKey) return j({ error: "No AI provider API key configured" }, 500, corsHeaders);
