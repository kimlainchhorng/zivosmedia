 ...
 * ---------------------
 * Server-gated owner/admin moderation for lodging reviews.
 */
import { createClient, serve } from "../_shared/deps.ts";
import { withSecurity } from "../_shared/withSecurity.ts";

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{12}$/i;
const ACTIONS = new Set(["reply", "set_flagged"]);
...
serve(withSecurity("lodging-review-manage", async (req, ctx) => {
  ...
  if (action === "set_flagged") {
    if (typeof body.flagged !== "boolean") return json({ error: "Invalid flagged status" }, 400);
    return updateReview(admin, reviewId, storeId, { flagged: body.flagged }, json);
  }

  const reply = cleanText(body.reply, 1, 2000);
  if (!reply) return json({ error: "Reply is required" }, 400);
  return updateReview(admin, reviewId, storeId, { reply, replied_at: new Date().toISOString() }, json);
...
async function updateReview(admin: any, reviewId: string, storeId: string, patch: Record<string, unknown>, json: (body: unknown, status?: number) => Response) {
  const { data, error } = await admin
    ...
    .single();
  if (error) {
    console.error("[lodging-review-manage:update]", error.message);
...
async function canManageStore(admin: any, userId: string, storeId: string): Promise<boolean> {
  const { data: store, error: storeError } = await admin
    ...
    .maybeSingle();
  if (storeError) {
    console.error("[lodging-review-manage:store]", storeError.message);
    return false;
  }
  ...
  if (roleError) {
    console.error("[lodging-review-manage:role]", roleError.message);
    return false;
  }
...
async function getReviewStoreId(admin: any, reviewId: string): Promise<string | null> {
  const { data, error } = await admin.from("lodging_reviews").select("store_id").eq("id", reviewId).maybeSingle();
  if (error) {
    console.error("[lodging-review-manage:review-store]", error.message);
    return null;
  }
