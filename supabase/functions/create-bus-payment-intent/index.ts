 ...
 * Mirrors create-ride-payment-intent: reserve funds now, capture when the
 * trip is confirmed. The booking row is created first via the
 * create_bus_booking RPC (status='hold'); this function attaches the
 * PaymentIntent and flips payment_status to 'authorized'.
 *
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Service-role read so we validate the booking authoritatively (amount is
