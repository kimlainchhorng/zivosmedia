...
Deno.serve(withSecurity("bot-api", async (req, ctx) => {
  const corsHeaders = ctx.corsHeaders;
  const apiHeaders = { ...corsHeaders, "Access-Control-Allow-Methods": "GET, POST, OPTIONS" };

  if (req.method === "OPTIONS") return new Response("ok", { headers: apiHeaders });
  ...
  try {
    ...
    if (!token) return j({ ok: false, error: "missing token" }, 401, apiHeaders);
    if (!method) return j({ ok: false, error: "missing method" }, 400, apiHeaders);

    const sb = supa();
    const { data: vrows } = await sb.rpc("verify_bot_token", { p_token: token });
    const auth = Array.isArray(vrows) ? vrows[0] : vrows;
    if (!auth) return j({ ok: false, error: "invalid token" }, 401, apiHeaders);
    if (!auth.is_active) return j({ ok: false, error: "bot inactive" }, 403, apiHeaders);
    ...
    switch (method) {
      case "getMe": {
        ...
        return j({ ok: true, result: data }, 200, apiHeaders);
      }
      case "sendMessage": {
        ...
        if (!chat_id || (!text && !image_url)) return j({ ok: false, error: "chat_id and text/image_url required" }, 400, apiHeaders);
        ...
        if (error) return j({ ok: false, error: error.message }, 500, apiHeaders);
        return j({ ok: true, result: data }, 200, apiHeaders);
      }
      case "setWebhook": {
        ...
        if (error) return j({ ok: false, error: error.message }, 500, apiHeaders);
        return j({ ok: true, result: { url: wurl || null } }, 200, apiHeaders);
      }
      case "deleteWebhook": {
        await sb.from("bots").update({ webhook_url: null }).eq("id", botId);
        return j({ ok: true, result: true }, 200, apiHeaders);
      }
      case "getUpdates": {
      ...
      case "setState": {
        ...
        if (!user_id) return j({ ok: false, error: "user_id required" }, 400, apiHeaders);
      ...
      case "getState": {
        ...
        return j({ ok: true, result: data ?? null }, 200, apiHeaders);
      }
      default:
        return j({ ok: false, error: `unknown method: ${method}` }, 400, apiHeaders);
    }
  } catch (e) {
    return j({ ok: false, error: String(e) }, 500, apiHeaders);
  }
