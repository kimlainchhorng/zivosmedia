 ...
 *  - Guest-callable only with a short-lived, booking-bound `deposit`
 *    capability; signed-in customers may use account ownership instead.
 *  - Only pending bookings with authoritative deposit_cents > 0 are payable.
    ...
    async (req, ctx) => {
      ...
      try {
        ...
        const { data: booking, error: bErr } = await supabase
          .from("salon_bookings")
          .select(
            "id, store_id, client_name, client_email, service_name, start_at, status, deposit_cents, deposit_paid_cents, stripe_checkout_session_id",
          )
          .eq("id", bookingId)
          .maybeSingle();
        if (bErr || !booking) {
        ...
        if (b.status !== "pending") {
          return new Response(
            JSON.stringify({
              error: `Booking is ${b.status}; deposits can only be paid on pending bookings.`,
            }),
            {
              status: 409,
              headers: { ...cors, "Content-Type": "application/json" },
        ...
        if (!accountId || (settings as any).stripe_status !== "active") {
          return new Response(
            JSON.stringify({
              error: "This salon hasn't finished Stripe onboarding yet.",
            }),
            {
              status: 409,
              headers: { ...cors, "Content-Type": "application/json" },
        ...
        if (b.stripe_checkout_session_id) {
          try {
            ...
            const existingMatchesBooking =
              existing.status === "open" &&
              !!existing.url &&
              existing.amount_total === b.deposit_cents &&
              existing.currency === "usd" &&
              existing.metadata?.type === "salon_deposit" &&
              existing.metadata?.salon_booking_id === b.id &&
              existing.metadata?.store_id === b.store_id;
            if (existingMatchesBooking) {
              return new Response(
        ...
        const { error: persistError } = await supabase
          ...
          .eq("id", b.id);
        if (persistError) {
          console.error("[create-salon-deposit:persist]", persistError.message);
          try {
            await stripe.checkout.sessions.expire(session.id);
          } catch (expireError) {
            console.error(
              "[create-salon-deposit:expire-orphan]",
              expireError instanceof Error ? expireError.message : "unknown",
            );
          }
          return new Response(
            JSON.stringify({ error: "Could not save the deposit session" }),
            {
              status: 500,
              headers: { ...cors, "Content-Type": "application/json" },
            },
