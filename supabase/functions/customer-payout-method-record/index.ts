...
const allowedRailHints = new Set([
  "aba",
  "bank_transfer",
  "bank_wire",
  "paypal",
]);
const canonicalRailByMethod: Record<string, string> = {
  aba: "aba",
      ...
      try {
        ...
        const execute = async () => {
          if (action === "delete") {
            ...
            if (!methodId)
              return { status: 400, body: { error: "Missing method_id" } };
          ...
          if (action === "set_default") {
            ...
            return {
              status: 200,
              body: { success: true, id: methodId, store_id: scope.storeId },
            };
          }
          ...
          const countryCode = body.country_code
            ? String(body.country_code).trim().toUpperCase().slice(0, 2)
            : null;
          const requestedRail = String(body.rail || "")
            .trim()
            .toLowerCase();
          const requestedMethod = String(body.method_type || requestedRail)
            .trim()
            .toLowerCase();
          const methodType =
            requestedMethod === "bank_wire" ? "bank_transfer" : requestedMethod;
          const rail = canonicalRailByMethod[methodType] || "";
          const label = String(body.label || defaultLabel(methodType))
            .trim()
            .slice(0, 120);
          const bankName = body.bank_name
            ? String(body.bank_name).trim().slice(0, 120)
            : null;
          const accountHolder = body.account_holder_name
            ? String(body.account_holder_name).trim().slice(0, 160)
            : null;
          const accountNumber = body.account_number
            ? String(body.account_number).trim().slice(0, 160)
            : null;
          const abaAccountId = body.aba_account_id
            ? String(body.aba_account_id)
                .replace(/[\s-]/g, "")
                .trim()
                .slice(0, 40)
            : null;
          const verificationNote = body.verification_note
            ? String(body.verification_note).trim().slice(0, 240)
            : null;
          const shouldDefault = body.is_default === true;

          if (!allowedMethods.has(methodType))
            return {
              status: 400,
              body: { error: "Invalid payout method type" },
            };
          if (requestedRail && !allowedRailHints.has(requestedRail)) {
            return { status: 400, body: { error: "Invalid payout rail" } };
          }
          if (methodType === "aba") {
            if (!accountHolder)
              return {
                status: 400,
                body: { error: "Account holder name is required" },
              };
          ...
          if (
            methodType === "paypal" &&
            (!accountNumber ||
              !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(accountNumber))
          ) {
            return {
              status: 400,
              body: { error: "Valid PayPal email required" },
            };
          }
          ...
          countQuery = storeId
            ? countQuery.eq("store_id", storeId)
            : countQuery.is("store_id", null);
          const { count } = await countQuery;
          const isDefault = shouldDefault || (count ?? 0) === 0;
          if (isDefault)
            await clearDefaultMethods(supabase, userId, storeId, methodType);
...
async function assertMethodOwner(
  supabase: any,
  userId: string,
  methodId: string,
) {
  ...
  if ((method as any).store_id)
    await assertCanManageStore(supabase, userId, (method as any).store_id);
  return {
    storeId: (method as any).store_id ?? null,
    methodType: (method as any).method_type,
  };
}

async function assertCanManageStore(
  supabase: any,
  userId: string,
  storeId: string,
) {
...
async function clearDefaultMethods(
  supabase: any,
  userId: string,
  storeId: string | null,
  methodType: string,
