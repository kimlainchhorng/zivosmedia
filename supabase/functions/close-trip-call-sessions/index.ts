...
async function attemptCloseTwilioSession(
  sessionSid: string,
  ...
  try {
    const r = await fetch(
      `https://proxy.twilio.com/v1/Services/${proxySid}/Sessions/${sessionSid}`,
      {
        method: "POST",
        headers: {
    ...
    if (r.ok)
      return { status: "closed", responseCode: r.status, errorMessage: null };
    if (r.status === 404)
      return {
        status: "not_found",
        responseCode: 404,
    ...
    return {
      status: "error",
      responseCode: null,
      errorMessage: `network: ${String(e).slice(0, 400)}`,
    };
  }
}
...
async function writeAudit(admin: any, row: any) {
  try {
    await admin.from("call_session_closure_audit").insert(row);
  } catch (e) {
...
async function closeWithRetry(
  ...
  let lastResult: CloseAttemptResult = {
    status: "error",
    responseCode: null,
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const retriable =
      code === null || code === 429 || (code >= 500 && code < 600);
    ...
    async (req, ctx) => {
      ...
      try {
        ...
        const { data: sessions } = await admin
          ...
          .select(
            "id, twilio_proxy_session_sid, ride_request_id, expires_at, closure_attempts, ride_requests!inner(status)",
          )
        ...
        for (const s of (sessions as any[]) || []) {
          ...
          const result = await closeWithRetry(
            admin,
            s.twilio_proxy_session_sid,
            s.ride_request_id,
          );
...
async function isInternalCaller(
  ...
  return isAuthorizedInternalCron(req, {
    functionName: "close-trip-call-sessions",
