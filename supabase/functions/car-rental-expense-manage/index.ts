 ...
 * -------------------------
 * Server-gated owner/admin CRUD for car-rental expense records.
 */
...
serve(withSecurity("car-rental-expense-manage", async (req, ctx) => {
  ...
  if (action === "delete") {
    const { error } = await admin
      ...
      .eq("store_id", storeId);
    if (error) {
      console.error("[car-rental-expense-manage:delete]", error.message);
  ...
  if (action === "update") {
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      console.error("[car-rental-expense-manage:update]", error.message);
  ...
  const { data, error } = await admin
    ...
    .single();
  if (error) {
    console.error("[car-rental-expense-manage:create]", error.message);
...
async function canManageStore(admin: any, userId: string, storeId: string): Promise<boolean> {
  const { data: store, error: storeError } = await admin
    ...
    .maybeSingle();
  if (storeError) {
    console.error("[car-rental-expense-manage:store]", storeError.message);
    return false;
  }
  ...
  if (roleError) {
    console.error("[car-rental-expense-manage:role]", roleError.message);
    return false;
  }
...
async function getExpense(admin: any, expenseId: string | null): Promise<{ store_id: string } | null> {
  ...
  const { data, error } = await admin
    ...
    .maybeSingle();
  if (error) {
    console.error("[car-rental-expense-manage:expense-store]", error.message);
    return null;
  }
  return data ?? null;
}
...
async function vehicleBelongsToStore(admin: any, vehicleId: string, storeId: string): Promise<boolean> {
  const { data, error } = await admin
    ...
    .maybeSingle();
  if (error) {
    console.error("[car-rental-expense-manage:vehicle]", error.message);
    return false;
  }
...
function cleanExpense(value: unknown, action: string):
  ...
  if ("description" in input) {
    const description = cleanText(input.description, 1, 250);
  ...
  if ("paid_to" in input) values.paid_to = cleanText(input.paid_to, 0, 160);
  if ("payment_method" in input) values.payment_method = cleanText(input.payment_method, 0, 160);
  ...
  if (action === "create") {
    for (const key of ["category", "description", "amount_cents", "expense_date"]) {
