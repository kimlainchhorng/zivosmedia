-- Scheduled pickup orders — let customers place an order for a future
-- time (catering, busy-morning pre-orders). Column on cafe_orders +
-- updated public RPC accepting p_scheduled_for. Server-side validation:
-- must be at least 5 minutes from now and no more than 7 days out.

ALTER TABLE public.cafe_orders
  ADD COLUMN IF NOT EXISTS scheduled_for TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS cafe_orders_store_scheduled_idx
  ON public.cafe_orders (store_id, scheduled_for)
  WHERE scheduled_for IS NOT NULL;

DROP FUNCTION IF EXISTS public.cafe_place_public_order(UUID, UUID, TEXT, JSONB, JSONB, TEXT, INTEGER, TEXT, TEXT, BOOLEAN);
...
BEGIN
  ...
  -- Validate scheduled pickup window: 5 min – 7 days out.
  IF p_scheduled_for IS NOT NULL THEN
    IF p_scheduled_for < v_now + interval '5 minutes' THEN
      RAISE EXCEPTION 'pickup time must be at least 5 minutes from now';
    END IF;
    IF p_scheduled_for > v_now + interval '7 days' THEN
      RAISE EXCEPTION 'pickup time can''t be more than 7 days out';
    END IF;
...
REVOKE ALL ON FUNCTION public.cafe_place_public_order(UUID, UUID, TEXT, JSONB, JSONB, TEXT, INTEGER, TEXT, TEXT, BOOLEAN, TIMESTAMPTZ) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.cafe_place_public_order(UUID, UUID, TEXT, JSONB, JSONB, TEXT, INTEGER, TEXT, TEXT, BOOLEAN, TIMESTAMPTZ) TO anon, authenticated;

-- Update cafe_auto_expire_pending_orders so it doesn't sweep scheduled
-- orders that haven't reached their pickup window yet.
  ...
  WITH updated AS (
    UPDATE public.cafe_orders
     ...
     WHERE status = 'pending'
       ...
       AND (scheduled_for IS NULL OR scheduled_for < now() - interval '60 minutes')
     RETURNING id
  )
