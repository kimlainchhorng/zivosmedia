-- Phase 63: bundles become orderable. The order RPC's items loop now
-- branches on `bundle_id`: a bundle row expands into individual order
-- items at their REGULAR price (never happy-hour-effective — bundles
-- don't compound with happy hour), and the savings vs the bundle's
-- flat price stack into v_discount alongside any promo/loyalty
-- redemption. Modifier IDs on a bundle row are ignored (MVP: no per-item
-- modifier picker inside a bundle).
...
DECLARE
  ...
  v_bundle RECORD; v_bline RECORD; v_bundle_sum INTEGER;
BEGIN
  ...
  FOR v_item_record IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Bundle branch: expand into individual cafe_order_items at the menu
    -- items' REGULAR price; stack savings into v_discount.
    IF v_item_record ? 'bundle_id' AND (v_item_record->>'bundle_id') IS NOT NULL THEN
      SELECT id, name, price_cents, store_id, is_active
        INTO v_bundle
        FROM public.cafe_bundles
        WHERE id = (v_item_record->>'bundle_id')::uuid;
      IF v_bundle.id IS NULL
         OR v_bundle.store_id <> p_store_id
         OR v_bundle.is_active = false THEN
        RAISE EXCEPTION 'bundle not available';
      END IF;
      ...
      FOR v_bline IN
        SELECT bi.menu_item_id, bi.quantity AS qty_per_bundle
          FROM public.cafe_bundle_items bi
          WHERE bi.bundle_id = v_bundle.id
          ORDER BY bi.sort_order
      LOOP
        SELECT id, name, price_cents, store_id, is_active, is_sold_out
          INTO v_menu
          FROM public.cafe_menu_items
          WHERE id = v_bline.menu_item_id;
        -- Orphan / inactive constituent: skip silently. Per the plan this
        -- keeps the bundle deliverable for the surviving items.
        IF v_menu.id IS NULL OR v_menu.store_id <> p_store_id OR v_menu.is_active = false THEN
          CONTINUE;
        END IF;
        IF v_menu.is_sold_out = true THEN
          RAISE EXCEPTION 'bundle item sold out';
        END IF;

        -- Bundles never compound with happy hour: regular price always.
        v_bundle_sum := v_bundle_sum + v_menu.price_cents * v_bline.qty_per_bundle;

        INSERT INTO public.cafe_order_items (
          order_id, store_id, menu_item_id, item_name,
          unit_price_cents, quantity, modifiers_total_cents, notes
        ) VALUES (
          v_order_id, p_store_id, v_menu.id, v_menu.name,
          v_menu.price_cents,
          v_bline.qty_per_bundle * v_qty,
          0,
          'Part of: ' || v_bundle.name
        );
      END LOOP;

      -- Clamp at 0 — never credit the customer on an upside-down bundle.
      v_discount := v_discount + GREATEST(v_bundle_sum - v_bundle.price_cents, 0) * v_qty;

      CONTINUE; -- next p_items record; skip the regular single-item path below
    END IF;

    -- Regular single-item path (unchanged from Phase 61's happy-hour version).
    SELECT id, name, price_cents, store_id, is_active, is_sold_out,
           happy_hour_price_cents, happy_hour_start, happy_hour_end
      INTO v_menu
    ...
    v_effective_price := CASE
      WHEN public.cafe_menu_item_happy_hour_active(
        v_menu.happy_hour_price_cents,
        v_menu.happy_hour_start,
        v_menu.happy_hour_end,
        v_now
      ) THEN v_menu.happy_hour_price_cents
      ELSE v_menu.price_cents
    END;
    ...
    VALUES (v_order_id, p_store_id, v_menu.id, v_menu.name, v_effective_price, v_qty, 0, NULLIF(v_item_record->>'notes', ''))
  ...
  IF p_promo_code IS NOT NULL AND length(trim(p_promo_code)) > 0 THEN
    ...
    IF v_promo.kind = 'percent' THEN
      v_discount := v_discount + LEAST(v_subtotal, FLOOR((v_subtotal * v_promo.amount)::NUMERIC / 100.0)::INTEGER);
    ELSE
      v_discount := v_discount + LEAST(v_subtotal, v_promo.amount);
    END IF;
