-- Phase 64: RPC honors bundle active windows. If the bundle has a window
-- and the current server-local time is outside it, the order is rejected
-- with 'bundle not available right now'. NULL window = always available.
...
BEGIN
  ...
  FOR v_item_record IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_item_record ? 'bundle_id' AND (v_item_record->>'bundle_id') IS NOT NULL THEN
      ...
      IF NOT public.cafe_bundle_window_active(
           v_bundle.active_start_time, v_bundle.active_end_time, v_now
         ) THEN
        RAISE EXCEPTION 'bundle not available right now';
      END IF;
    ...
    -- Regular single-item path (unchanged).
    SELECT id, name, price_cents, store_id, is_active, is_sold_out,
