-- Extend cafe_place_public_order to snapshot happy hour pricing when
-- active. Diff vs prior version: SELECT pulls the 3 new columns, and we
-- compute v_effective_price using cafe_menu_item_happy_hour_active().
...
BEGIN
  ...
  FOR v_item_record IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Use happy hour price when active; otherwise fall back to regular.
