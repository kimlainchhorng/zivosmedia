-- Restore the chat media storage read policy to Storage-compatible
-- expressions after album item path checks caused Storage API validation
-- failures. Locked albums remain unlockable through the RPC; album item reads
-- are intentionally left to owner-prefixed paths until a private-bucket access
-- route is added.
