...
| **SMS audit log — ✅ APPLIED via extract 2026-09-04** | ~~`sms_send_log`~~ | new `20260904210000_sms_send_log_extract.sql` (parent file NOT applied: it create-or-replaces the LIVE `enqueue_notification` and adds behavioral triggers to live hot tables — notification pipeline activation is an owner decision) | `send-sms` (deployed) now has its audit table; `AdminNotificationAnalyticsPage` |
| **Login protection — ⚠ OWNER-ONLY** | `auth_login_protection`, `auth_login_events` absent | `20260411170000` REPLACES the LIVE `auth_record_login_attempt` (currently writing to healthy `login_attempts`) and `auth_precheck_login` — applying changes live auth/lockout behavior. Deliberately not touched | owner decision with security review |
| **Group message reports / sensitive media — ✅ SCHEMA APPLIED 2026-09-04** | ~~3 report tables~~ + sensitivity columns on live tables | `20260526165000` applied whole (fully guarded, zero data statements): comment/chat/group report tables + `user_posts` sensitivity columns + visibility indexes | chat moderation + sensitive-media UI now schema-backed |
