# MiMo run — 2026-06-14T08:26:51.977Z
...
**Plan:** 5 raw `<button>` controls, 5 className edits, 3 new aria attrs, 0 logic lines. One ring-color migration (A), two transition-colors→transition-all flips (B/C), one no-flip append (D), one ring-only append (E). All selectors/queries/state/setters byte-identical.

---

**Proposed diff — `src/pages/account/AccountAnalyticsPage.tsx`**

```diff
--- a/src/pages/account/AccountAnalyticsPage.tsx
+++ b/src/pages/account/AccountAnalyticsPage.tsx
@@ (A) L184 — HEADER BACK button: MIGRATE ring-primary/60 → ring-ring (KEEP scale-95, bare transition, outline-none, aria-label)
...
@@ (B) L196 — PERIOD filter chip (×3): FLIP transition-colors→transition-all + ADD aria-pressed + APPEND scale/ring
-            <button type="button"
+            <button type="button"
               key={p}
               onClick={() => setPeriod(p)}
...
@@ (C) L239 — BUCKET filter chip (×2): identical treatment to B
-                <button type="button"
+                <button type="button"
                   key={b.id}
                   onClick={() => setTopBucket(b.id)}
...
@@ (D) L257 — METRIC filter chip (×4): NO FLIP (already transition-all) + ADD aria-pressed + APPEND scale/ring
-                  <button type="button"
+                  <button type="button"
                     key={m.id}
...
@@ (E) L287 — TOP-POST ROW button: ring-ONLY (KEEP hover/active bg + transition-colors, NO scale, NO flip)
               <button type="button"
                 key={post.id}
...
| # | Control | Ring color + reason | Press tier | Transition | Ring placement | Aria |
|---|---------|-------------------|-----------|-----------|---------------|------|
| A | Header back (icon-only) | `ring-ring` (MIGRATE from off-house `ring-primary/60`; neutral header) | KEEP `scale-95` (existing) | KEEP bare `transition` (covers transform+colors) | OUTWARD (neutral header) | KEEP `aria-label="Back"` |
| B | Period chip ×3 | `ring-ring` (bg-ig-gradient selected on neutral header) | `[0.97]` segmented-filter | **FLIP** `transition-colors`→`transition-all` (new scale + gradient wash) | OUTWARD (neutral header) | **ADD** `aria-pressed={period === p}` |
| C | Bucket chip ×2 | `ring-ring` (bg-ig-gradient selected on neutral bg-card panel) | `[0.97]` segmented-filter | **FLIP** `transition-colors`→`transition-all` | OUTWARD (neutral bg-card) | **ADD** `aria-pressed={topBucket === b.id}` |
| D | Metric chip ×4 | `ring-ring` (faint bg-tint selected on neutral bg-card; NOT inset — pill chip in scroller) | `[0.97]` segmented-filter | NO FLIP (already `transition-all`) | OUTWARD (pill chip, not flush-media/inset case) | **ADD** `aria-pressed={active}` |
| E | Top-post row (mapped) | `ring-ring` (neutral bg-card row, text + small h-12 thumbnail, NOT image-dominant) | NO scale (KEEP existing `active:bg-muted/40` press wash) | NO FLIP (leave `transition-colors`) | **INSET** (flush child of `rounded-2xl overflow-hidden` section) | none |

**Guard-grep summary:**
- `className` edits: 5 (`−`/`+` pairs)
- New aria attrs: 3 (`aria-pressed` ×3 on B/C/D)
- `aria-label` new: 0 (A's pre-existing kept)
- `focus-visible:ring` = 5, `ring-ring` = 5, `ring-inset` = 1 (E), `ring-white/70` = 0
- `active:scale-95` = 0 new (A pre-existing kept), `active:scale-[0.97]` = 3 new (B/C/D)
- FLIPs = 2 (B/C `transition-colors`→`transition-all`)
- NEW transitions = 0 (B/C are FLIPs; D/E/A untouched)
- `transition-colors` removed = 2 (B/C flipped)
- Logic lines touched: 0

**LEAVE (confirmed):**
- Metrics-grid stat `<div>`s (non-interactive)
- Follower-growth chart bar `<div>`s (non-interactive, title tooltips)
- Top Posts `section`/`header`/`h2`/`h3` + count badges + caption/media-type `<p>`s + timeZone `<span>` (non-interactive)
- Post thumbnail `<img>` (decorative `alt=""`) + Calendar fallback
- All icons (decorative)
- `ZivoMobileNav` (own file)
- `SEOHead` (head)
- `Button` import (unused after edits — actually it's not used on this page at all, was already unused)

**Owner verification:**
1. Run `npm run update` — must pass (0 type errors + build).
2. Preview at 375/768/1280: the period/bucket chips should show press-scale + focus rings on Tab; the metric chips should show press-scale + focus rings; the top-post rows should show inset focus rings on Tab; the back button should show a black ring (not purple) on focus.
3. `aria-pressed` toggles correctly on the segmented filters (inspect DevTools).
4. No logic changes — all `setPeriod`/`setTopBucket`/`setTopMetric`/`navigate`/`useQuery` byte-identical.
