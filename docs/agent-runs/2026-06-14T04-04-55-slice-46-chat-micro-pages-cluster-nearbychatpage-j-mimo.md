# MiMo run — 2026-06-14T04:04:55.553Z
...
Two chat micro-pages, three RAW button controls total, zero logic changes. Parity with the 40+ prior slices.

| Control | File | Action | Scale tier | Transition | Ring color | ARIA |
|---------|------|--------|-----------|------------|------------|------|
| **(A)** Back | `NearbyChatPage.tsx` L28 | append tokens + ADD `aria-label` | icon-only `scale-95` | FRESH `transition-all` (`hover:bg-muted/60`) | OUTWARD `ring-ring` (neutral header) | **ADD** `aria-label="Back"` |
| **(B)** Message | `NearbyChatPage.tsx` L71 | append tokens | icon-only `scale-95` | `transition-TRANSFORM` (NO hover color) | OUTWARD `ring-ring` (gradient fill on neutral bg-card row) | **KEEP** `aria-label="Message"` |
| **(C)** Back | `JoinGroupPage.tsx` L27 | append tokens | icon-only `scale-95` | FRESH `transition-all` (`hover:bg-muted/60`) | OUTWARD `ring-ring` (neutral header) | **KEEP** `aria-label="Back"` |

SKIP: shadcn `Switch` (NearbyChat L45), shadcn `Button` ×3 (JoinGroup L113/155/162), all presentational `<div>`s, Avatars, icons, text. No keyboard-inaccessible clickable `<div>`s in either file.
...
**(A)** ADD `aria-label="Back"` (icon-only, none present — genuine a11y fix) + FRESH `transition-all` (has `hover:bg-muted/60` color fade, no prior transition) + `active:scale-95` (icon-only tier) + OUTWARD `ring-ring` (sticky header, not overflow-hidden). ✅

**(B)** `transition-TRANSFORM` NOT `transition-all` — the button has **NO** hover color/opacity/underline; it's a pure gradient-filled press-scale. `active:scale-95` (icon-only tier). KEEP existing `aria-label="Message"`. `ring-ring` NOT `ring-white/70` — this is a gradient-**filled** button sitting on a **neutral** `bg-card` row (`div className="... p-3 rounded-2xl bg-card border border-border/30"`), NOT overflow-hidden; the outward ring renders against the neutral parent, not the gradient fill. ✅

**(C)** KEEP existing `aria-label="Back"` + FRESH `transition-all` (has `hover:bg-muted/60`) + `active:scale-95` (icon-only tier) + OUTWARD `ring-ring` (sticky header, not overflow-hidden). ✅

**SKIP lists confirmed:** shadcn `Switch` (NearbyChat L45) ✅; shadcn `Button` "Back to chats" (L113) ✅; shadcn `Button` "Cancel" (L155) ✅; shadcn `Button` "Join group" (L162) ✅; all presentational `<div>`s (nearby-user rows, group-avatar wrapper) ✅; all Avatars/icons/text ✅.

**No keyboard-inaccessible clickable `<div>`s** in either file. ✅

**Keyboard/responsive flags:** None. Both pages are simple vertical stacks with `px-4` padding, `flex-1 min-w-0` text columns with `truncate`, and small `h-9`/`w-9` icon buttons — no crush at any viewport. The sub-44px tap targets (`h-9` ~36px, `p-1.5` ~24px) follow the repo-wide compact pattern — flag but don't fix.

**Hard-rule diff totals:** 3 className line swaps + 1 new `aria-label` (A), 0 logic lines touched. All `onClick` handlers / `disabled` props / data hooks preserved byte-for-byte.

**Verify:** `npm run update` must pass (type-check + worker type-check + production build, exit 0) before marking done.
