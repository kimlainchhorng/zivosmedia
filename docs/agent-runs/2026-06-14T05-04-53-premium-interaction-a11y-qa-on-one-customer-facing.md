# DeepSeek run — 2026-06-14T05:04:53.429Z
...
Looking at the file, I can see the Unmute button already has the ring classes applied (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring`). This page is already compliant with the reference standard.

## Assessment

**(1) RING-ONLY don't-churn:** ✅ Already done. The Unmute button at L132 already has `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring`. No change needed. `active:scale-95` and `transition-all` are correctly kept.

**(2) NO aria-pressed:** ✅ Correct. One-shot unmute action — the row is optimistically removed from the DOM. Not a persistent toggle.

**(3) Inventory exhaustive:** ✅ Correct. Exactly 1 RAW Unmute button + 1 shadcn back Button. Hero stat `motion.div` (L98) and mute-row `motion.div` (L121) are presentational (no onClick). Loading skeletons (L106) and empty-state card (L108) are non-interactive.

**(4) Responsive @375:** ✅ Clean. The flex row `flex items-center gap-3 p-3` with `flex-1 min-w-0` on the text column and `shrink-0` on the Unmute pill works fine at 375px. The `line-clamp-1` on the conversation_id prevents overflow. Sub-44px tap targets (back `h-10 w-10` = 40px, Unmute `h-8` = 32px) are repo-wide compact pattern — flag but don't fix.

**(5) Owner flags:** ✅ Correct. Truncated conversation_id (no human-readable name), no tap-through to chat, supabase cast workaround, optimistic rollback via invalidateQueries — all existing behavior, out of scope.

## Proposed change

**No diff needed.** The page already meets the reference standard. The Unmute button at `src/pages/MutedChatsPage.tsx:132` already has:
```
...
Run `npm run update` to confirm the build passes with no changes.
