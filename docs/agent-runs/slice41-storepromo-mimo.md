# MiMo run — 2026-06-14T03:42:30.290Z
...
Two edits on `src/pages/StorePromoCodesPage.tsx` — both className-only, zero logic changes.

### (1–3) Answers

| # | Verdict |
|---|---------|
| **(1)** | **FULL link treatment — agree with your instinct.** `hover:underline` is a hover decoration → `transition-all` per established convention (eases the new scale; underline itself is binary but the signal is "has hover feedback"). Link tier `[0.97]`. Visible text → NO `aria-label`. Navigational → NO `aria-pressed`. OUTWARD ring (flex-1 column inside `p-3.5 rounded-2xl` card, NOT overflow-hidden). |
| **(2)** | **RING-ONLY, confirmed.** DON'T-CHURN — pre-existing `transition-all` + `active:scale-[0.98]` is a valid full-width-tier combo. NO `aria-pressed` (transient 1.5s copy-confirmation + label text changes from code → "Copied" — NOT a persistent toggle). `[0.98]` correct, don't renumber. OUTWARD ring. |
| **(3)** | **Inventory exhaustive.** 2 RAW (`<button>`) + 1 shadcn back (SKIP). 0 `motion.button`. Hero stat `motion.div` (L109) + promo-row `motion.div` (L141) both presentational (NO onClick). Skeletons (L121) + empty-state (L123) non-interactive. |
| **(4)** | **Responsive clean.** `max-w-2xl mx-auto px-4` → ~343px @375px. Promo card: logo 40px + gap 12px + `flex-1 min-w-0` (store-name `line-clamp-1` + value `text-2xl` + meta `flex-wrap`) then full-width Copy `h-10 w-full` below → no crush. **Sub-44px FLAG (repo-wide):** store-name link inline text ~20px height (established compact text-link pattern); back `h-10 w-10` ~40px ✓; Copy `h-10` ~40px ✓. |
| **(5)** | **Owner flags:** (a) store-name link is no-op when `s?.slug === null` → data issue, FLAG not fix; (b) Copy button has no `aria-live` region for "Copied" announcement — visible text change is sufficient but `aria-live="polite"` on the button text would be a nice a11y enhancement → FLAG (optional, out of scope); (c) store-name link sub-44px tap target → FLAG (repo-wide pattern). |

---

### Proposed diff

**File:** `src/pages/StorePromoCodesPage.tsx`
...
**Summary:** 2 className-only line edits, 0 new attributes, 0 logic changes.  
- **(A)** store-name link: +`transition-all` +`active:scale-[0.97]` +`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring`  
- **(B)** Copy-code button: +`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` only (DON'T-CHURN)

### Owner verification

Run `npm run update` — must exit 0 (type-check + worker type-check + production build). Preview at `/store-promos` (or navigate via app) at 375/768/1280 widths. Two keyboard-focusable controls now receive visible focus rings; the store-name link gets a subtle press-scale on click/tap.
