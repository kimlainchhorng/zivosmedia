# DeepSeek run — 2026-06-13T20:57:18.497Z
...
1. **Replace the header** — swap the plain badge-row + separate `<h1>` with the reference pattern: a `rounded-2xl bg-primary/10` icon chip (Mic icon), the topic as the title, and a right-side Live/Ended badge stack. Keep the small "Voice room" label.
2. **Add a polished meta row** — `flex flex-wrap text-xs text-muted-foreground` with inline-flex icon spans showing:
   - Start/end time (relative: "Started 10:30 AM" when live, "Ended 11:45 AM" when ended)
   - Participant count with Users icon
   - "Voice room" label with Radio icon
3. **Add `border-t border-border/40` separation** around the Actions section.
4. **Keep the host card and participants strip** — they already sit cleanly in the layout.
5. **No changes** to supabase queries, mutations, react-query keys, routing, GroupCallLauncher wiring, or the 10s participants polling.

## Proposed Change

**File:** `src/pages/hubs/VoiceRoomDetailPage.tsx`

Replace the room-loaded branch (the `div` with `className="space-y-5"`) with this:

```tsx
<div className="space-y-5">
  {/* Header: icon chip + title + Live/Ended badge */}
  <div className="flex items-start gap-3">
    ...
    {room.is_live ? (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500 text-white text-[10px] font-extrabold uppercase shrink-0">
        <span className="h-1.5 w-1.5 rounded-full bg-white animate-pulse" />
        Live
      </span>
    ) : (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-muted text-foreground text-[10px] font-extrabold uppercase shrink-0">
        Ended
      </span>
    )}
  </div>

  {/* Meta row: start/end time + participants + voice room label */}
    ...
    {room.started_at && (
      <span className="inline-flex items-center gap-1">
        ...
        {room.is_live
          ...
          : `Ended ${room.ended_at ? new Date(room.ended_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : ''}`}
      </span>
    )}
    <span className="inline-flex items-center gap-1">
      <Users className="w-3.5 h-3.5" /> {participants.length} listening
    </span>
    <span className="inline-flex items-center gap-1">
  ...
  {/* Actions — separated with border-t */}
...
- **No supabase queries, mutations, react-query keys, routing, GroupCallLauncher wiring, or polling changed** — only JSX/Tailwind and display-only time formatting.
- **`started_at` and `ended_at` now shown** — relative time display ("Started 10:30 AM" / "Ended 11:45 AM") in the meta row.
- **Header matches reference pattern** — icon chip + title + right-side badge stack (Live/Ended).
- **`border-t border-border/40`** separates the Actions section.
- **`active:scale-[0.98]`** preserved on all buttons.
