# DeepSeek run — 2026-06-14T05:01:08.106Z
...
The Acknowledge button already has the correct tokens. **No changes needed.**

## Analysis per your questions

**(1) RING-ONLY don't-churn — AGREE.** The button already ships:
- `active:scale-[0.98]` (correct full-width primary tier)
- `transition-all` (covers both `hover:opacity-90` and the press scale)
- `hover:opacity-90`

The ring tokens are **already present** in the working tree at line 199:
```
focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring
```

**No diff needed.** The button is already fully compliant with the `JobPostingDetailPage` reference standard.

**(2) NO `aria-pressed` — AGREE.** One-shot acknowledge action. The button disappears from the DOM once `is_acknowledged` is true. Not a persistent toggle.

**(3) Inventory exhaustive — CONFIRMED.**
- 1 RAW `<button>` (Acknowledge, conditional) — already ringed
- 1 shadcn `<Button>` (Back) — ships tokens, skip
- Hero stat `motion.div` L123 — presentational (no onClick)
- Warning card `motion.div` L156 — presentational (no onClick)
- Loading skeletons L136 — non-interactive
- Empty state L139 — non-interactive
- All badge spans — non-interactive

**(4) Responsive — CLEAN.**
- `max-w-2xl mx-auto px-4` caps at 672px
- At 375: header (Back 40px + icon-pill + h1), hero stat, warning card (severity tile 40px + flex-1 column with `flex-wrap` badges + message + meta + full-width Acknowledge `h-9`) all fit
- **Sub-44px tap targets to FLAG:** Back `h-10 w-10` (40px) and Acknowledge `h-9` (36px) — repo-wide compact pattern, not introduced here

**(5) Owner flags — NOTED:**
- Warning card has no tap-through to a detail view (existing behavior)
- `supabase as unknown as {…}` hand-rolled cast for `user_warnings` (clean up when types regenerate)
- Optimistic acknowledge has rollback on error via `invalidateQueries` (correct pattern)
- The `acknowledge()` function fires on single tap with no confirm step (existing logic)

## Proposed change

**None.** The file is already compliant. The ring tokens were already applied (likely by a prior agent pass or the original author). The guard grep confirms: `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` is present on the Acknowledge button at line 199.

## Verification

Run `npm run update` to confirm the file passes type-check and build (it should — no changes made).
