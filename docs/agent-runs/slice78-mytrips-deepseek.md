# DeepSeek run — 2026-06-14T08:32:50.897Z
...
# MyTripsPage.tsx — Premium Interaction + A11y Pass

## Plan

5 raw interactive controls need focus rings + aria. Key decisions:

| Control | Press handling | Ring placement | Transition | Aria |
|---------|---------------|----------------|------------|------|
| **A** Service chips (×5) | `whileTap` present → ring-ONLY, no CSS scale | OUTWARD `ring-ring` (gap-2 padded container, NOT overflow-hidden) | Already `transition-all` → no flip | `aria-pressed` |
| **B** Status tabs (×5) | `whileTap` present → ring-ONLY, no CSS scale | OUTWARD `ring-ring` (p-1.5 padding = 6px, ring won't clip at overflow-hidden edge) | Already `transition-all` → no flip | `aria-pressed` |
| **C** Header Back Link | Wrapper `motion.div` `whileTap` → ring-ONLY, no CSS scale | OUTWARD `ring-ring` (neutral header bg) | Already `transition-all` → no flip | Keep existing `aria-label` |
| **D** "View" Link | Already has CSS `active:scale-95` → ring-ONLY, KEEP existing scale | OUTWARD `ring-ring` (faint primary-tint parent = neutral-ish, NOT saturated/dark) | Already `transition-all` → no flip | No aria (visible text) |
| **E** TripCard wrapping Link | Inner `motion.div` `whileTap` → ring-ONLY, no CSS scale | OUTWARD `ring-ring` (neutral page bg) | No transition needed (ring-only) | No aria (visible card content) |

## Diff

```diff
--- a/src/pages/app/MyTripsPage.tsx
+++ b/src/pages/app/MyTripsPage.tsx
...
@@ -290,6 +290,7 @@ export default function MyTripsPage() {
...
+                      aria-pressed={serviceFilter === filter.id}
                     >
                       ...
                       {filter.label}
@@ -312,7 +313,7 @@ export default function MyTripsPage() {
                     <motion.button
                       key={filter.id}
                       whileHover={{ scale: 1.04 }}
                       whileTap={{ scale: 0.92 }}
...
@@ -320,6 +321,7 @@ export default function MyTripsPage() {
...
@@ -248,7 +248,7 @@ function NextTripCard({ trips }: { trips: UnifiedTrip[] | undefined }) {
...
@@ -141,7 +141,7 @@ function TripCard({ trip, index }: { trip: UnifiedTrip; index: number }) {
...
| Control | Edit | Aria | Ring | Press | Transition |
|---------|------|------|------|-------|------------|
| **A** Service chips (L285) | Append `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` + add `aria-pressed={serviceFilter === filter.id}` | `aria-pressed` | OUTWARD `ring-ring` (gap-2 padded container, not overflow-hidden) | ring-ONLY (whileTap present) | No flip (already `transition-all`) |
| **B** Status tabs (L315) | Append `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` + add `aria-pressed={statusFilter === filter.id}` | `aria-pressed` | OUTWARD `ring-ring` (p-1.5=6px padding, ring won't clip at overflow-hidden edge) | ring-ONLY (whileTap present) | No flip (already `transition-all`) |
| **C** Header Back Link (L250) | Append `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` | Keep existing `aria-label="Go back"` | OUTWARD `ring-ring` (neutral header bg) | ring-ONLY (wrapper whileTap) | No flip (already `transition-all`) |
| **D** "View" Link (L188) | Append `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring` | No aria (visible text) | OUTWARD `ring-ring` (faint primary-tint parent = neutral-ish, not saturated/dark) | ring-ONLY (keep existing `active:scale-95`) | No flip (already `transition-all`) |
| **E** TripCard wrapping Link (L141) | Add `className="block rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"` | No aria (visible card content) | OUTWARD `ring-ring` (neutral page bg) | ring-ONLY (inner motion whileTap) | No transition needed (ring-only) |

## Key decisions resolved

1. **A/B ring-ONLY**: Both have framer-motion `whileTap` → no competing CSS `active:scale` added. ✅
2. **B OUTWARD not INSET**: The `GlassCard3D` is `overflow-hidden` BUT the tabs sit inside `p-1.5` (6px) padding with `gap-0.5` (2px) between them. A 2px outward ring renders within that padding, NOT clipped at the card edge. ✅
3. **D `ring-ring` not `ring-white/70`**: The parent is a `bg-gradient-to-br from-primary/15 via-primary/8 to-primary/5` banner — a faint tint, NOT a saturated/dark/image surface. The outward ring renders against this faint-tint parent → `ring-ring`. ✅
4. **E ADD className on bare Link**: Follows the SupportCenterPage precedent — a natively-focusable `<a>` with no visible focus indicator gets a display-only `block rounded-2xl focus-visible:ring-ring` className. No `role`/`tabIndex`/`onKeyDown`. ✅

## Owner verification

- Run `npm run update` to verify type-check + build pass
- Preview at `/app/trips` on mobile (375px) and desktop (1280px) to confirm focus rings render correctly
- Verify the 5 service chips and 5 status tabs all get `aria-pressed` toggling correctly
- Confirm the TripCard wrapping `<Link>` doesn't break the card layout (the `block` className makes the `<a>` fill the card width)
