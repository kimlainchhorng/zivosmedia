# MiMo run — 2026-06-14T08:18:16.396Z
...
5 raw `<button>`s in scope. All decisions confirmed against your analysis. Here is the exact diff.

---

## Proposed diff

**File:** `src/pages/account/AccountSubscriptionsPage.tsx`
...
| Control | Ring | Press tier | Transition | aria-* | Key reasoning |
|---------|------|-----------|------------|--------|---------------|
| **A** Back (L141) | OUTWARD `ring-ring` | icon-only `scale-95` | `transition-all` **NEW** | keep `aria-label="Back"` | `hover:bg-muted/60` present → both hover color + new scale animate → `transition-all`, not `transition-transform` |
| **B** Tab ×2 (L165) | OUTWARD `ring-ring` | segmented `scale-[0.97]` | already `transition-all` → **NO FLIP** | `aria-pressed={tab === t}` **NEW** | Track has `p-1` padding, NOT `overflow-hidden` → outward ring renders in the padding; already `transition-all` → append only |
| **C** Creator-row (L220) | **INSET** `ring-ring` | ring-only, **NO scale** | leave `transition-colors` | none | Flush child of `rounded-2xl overflow-hidden` card → inset; bg-card row with small avatar, NOT image-dominant → `ring-ring` (not `ring-white/70`); already `active:bg-muted/30` → don't add competing scale |
| **D** Cancel (L273) | OUTWARD `ring-ring` | text-link `[0.97]` | `transition-transform` **NEW** | none | `hover:underline` is text-decoration, not animatable → scale is sole animated prop → `transition-transform` |
| **E** Resubscribe (L282) | OUTWARD `ring-ring` | text-link `[0.97]` | `transition-transform` **NEW** | none | Identical to D |

## Diff stats

- 5 className edits + 1 `aria-pressed` attribute (B)
- `focus-visible:ring` = 5, `ring-ring` = 5 (A/B/D/E OUTWARD, C INSET)
- `active:scale-95` = 1 new (A), `active:scale-[0.97]` = 3 new (B/D/E)
- NEW transitions = 3 (A `transition-all`, D/E `transition-transform`)
- FLIPs = 0 (B already `transition-all`, C untouched)
- 0 logic lines touched — every `onClick`/`navigate`/`setTab`/`setConfirmCancel`/`useQuery`/`useMutation`/edge-function invoke byte-identical

## LEAVE list (confirmed)

- shadcn `<Button>` L195 (own tokens)
- All `AlertDialog*` L299–322 (own tokens)
- `Avatar`/`AvatarImage`/`AvatarFallback` L225 (own tokens)
- `motion.div` card wrapper L214 (entrance anim, no onClick)
- `ZivoMobileNav` L324 (own file)
- All static `<p>`/`<span>` (monthly spend, tier badges, dates, counts)
- All icons (ArrowLeft, Heart, Crown, Loader2, Sparkles, Calendar, AlertCircle)

## Owner verification

1. Run `npm run update` — must pass (types + worker types + production build, TRUE EXIT 0).
2. Preview at 375/768/1280 — verify focus rings appear on Tab/Back/creator-row/Cancel/Resubscribe when tabbed; verify no visual regressions on the AlertDialog cancel flow.
3. Commit when ready.
