...
| `zivodriver.com` serves the generic feed | ❌ separate app/deploy | Driver now belongs to the dedicated `zivodriver` repo/deployment, not the Zivosmedia artifact. | ❌ **No** — deploy Driver and bind the domain there. See `docs/ZIVODRIVER_DEPLOYMENT_ROUTING_RUNBOOK.md` / `INFRA_FIX_RUNBOOKS.md` Runbook A. |
| `zivobusiness.com` / `zivoemployee.com` serve the generic feed | ✅ host aliases in Zivosmedia | Business/Employee alias hosts are served by the Zivosmedia build/routes. | ✅ Yes, after current Zivosmedia main is built, deployed, and bound to those aliases. |
