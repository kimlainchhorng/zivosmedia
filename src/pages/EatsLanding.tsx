...
import {
  ...
  useEatsMenu,
  type EatsCartItem,
} from "@/hooks/useEatsData";
import { supabase } from "@/integrations/supabase/client";
import { Capacitor } from "@capacitor/core";
import { KHR_PER_USD } from "@/lib/currency";
import { useEatsOrder, type PlaceOrderResult } from "@/hooks/useEatsOrder";
...
import EatsInlinePaymentForm from "@/components/eats/EatsInlinePaymentForm";
import {
...
type EatsCardPaymentSession = Extract<
  PlaceOrderResult,
...
export default function EatsLanding() {
  ...
  useEffect(() => {
    ...
    const apply = (status?: string | null) => {
      ...
      if (typeof step === "number")
        setStatusStep((prev) => Math.max(prev, step));
    };
    supabase
      .from("food_orders")
      .select("status")
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setTrackedOrderId(null);
    };
  ...
  const { data: menuItems = [], isLoading: loadingMenu } =
    useEatsMenu(selectedRestaurantId);
  ...
  const [sortBy, setSortBy] = useState<
    "recommended" | "rating" | "time" | "nearby"
  >("recommended");
  ...
  const [currency, setCurrency] = useState<"USD" | "KHR">(() => {
    try {
      return (localStorage.getItem(CURRENCY_KEY) as "USD" | "KHR") || "USD";
    } catch {
      return "USD";
    }
  });
  ...
  const [orderMode, setOrderMode] = useState<"delivery" | "pickup">(() => {
    try {
      return (
        (localStorage.getItem(ORDER_MODE_KEY) as "delivery" | "pickup") ||
        "delivery"
      );
    } catch {
      return "delivery";
  ...
  const requestNotifications = async () => {
    try {
      ...
      if (r === "granted")
        toast.success(
          "Notifications enabled — we'll keep you posted on every order",
        );
    } catch {}
  };
  useEffect(() => {
  ...
  const [activeDietary, setActiveDietary] = useState<string | null>(() => {
    try {
      return localStorage.getItem(DIETARY_KEY);
    } catch {
      return null;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(ADDRESS_KEY, deliveryAddress);
    } catch {}
  }, [deliveryAddress]);
  ...
  const [contactlessDelivery, setContactlessDelivery] = useState<boolean>(
    () => {
      try {
        return localStorage.getItem(CONTACTLESS_KEY) === "1";
      } catch {
        return false;
      }
    },
  ...
  useEffect(() => {
    try {
      localStorage.setItem(CONTACTLESS_KEY, contactlessDelivery ? "1" : "0");
    } catch {}
  }, [contactlessDelivery]);
  const TIP_PREF_KEY = "zivo:eats:tipPref";
  const [selectedTip, setSelectedTip] = useState<string>(() => {
    try {
      return localStorage.getItem(TIP_PREF_KEY) || "20";
    } catch {
      return "20";
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem(TIP_PREF_KEY, selectedTip);
    } catch {}
  }, [selectedTip]);
  const CUSTOM_TIP_KEY = "zivo:eats:customTip";
  const [customTipAmount, setCustomTipAmount] = useState<string>(() => {
    try {
      return localStorage.getItem(CUSTOM_TIP_KEY) || "";
    } catch {
      return "";
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem(CUSTOM_TIP_KEY, customTipAmount);
    } catch {}
  }, [customTipAmount]);
  ...
  >({});
  const [paymentType, setPaymentType] = useState<EatsPaymentRail>("card");
  const [cardPaymentSession, setCardPaymentSession] =
    useState<EatsCardPaymentSession | null>(null);
  const [cancellingCardPayment, setCancellingCardPayment] = useState(false);

  // Favorites — persisted via localStorage and shared across the app
  const { favorites, toggle: toggleFavoriteHook } =
  ...
  const filtered = useMemo(() => {
    return restaurants
      .filter((r) => {
        ...
        if (activeDietary) {
          const haystack =
            `${r.name || ""} ${r.cuisine_type || ""} ${(r as { description?: string | null }).description || ""} ${((r as { tags?: string[] | null }).tags || []).join(" ")}`.toLowerCase();
          const needles =
            activeDietary === "glutenfree"
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "time")
          return (a.avg_prep_time ?? 30) - (b.avg_prep_time ?? 30);
        if (sortBy === "nearby" && userCoords) {
          const ar = a as { lat?: number | null; lng?: number | null };
          const br = b as { lat?: number | null; lng?: number | null };
          const da =
            ar.lat != null && ar.lng != null
              ? haversineKm(userCoords, { lat: ar.lat, lng: ar.lng })
              : Infinity;
          const db =
            br.lat != null && br.lng != null
              ? haversineKm(userCoords, { lat: br.lat, lng: br.lng })
              : Infinity;
          return da - db;
        }
  ...
  const deliveryFee =
    currentRestaurant?.delivery_fee_cents != null
      ? currentRestaurant.delivery_fee_cents / 100
      : 3.99;
  ...
  const speedExtra =
    deliverySpeedOptions.find((o) => o.id === selectedSpeed)?.extraCost ?? 0;
  const promoDiscount = useMemo(() => {
    if (!promoApplied || !promoData) return 0;
    if (promoData.discount_amount_cents)
      return Math.min(promoData.discount_amount_cents / 100, cartTotal);
    if (promoData.discount_percent)
      return (
        Math.round(cartTotal * (promoData.discount_percent / 100) * 100) / 100
      );
    return 0;
  ...
  const checkoutDeliveryFee =
    orderMode === "pickup" || cartTotal >= 20 ? 0 : deliveryFee;
  const checkoutTipAmount = orderMode === "delivery" ? tipAmount : 0;
  const checkoutSpeedExtra = orderMode === "delivery" ? speedExtra : 0;
  const grandTotal =
    Math.round(
      (cartTotal +
        checkoutDeliveryFee +
  ...
  const handleBack = () => {
    if (step === "checkout") setStep("cart");
      ...
      setStep(selectedRestaurantId ? "restaurant" : "browse");
  ...
  const handlePlaceOrder = async () => {
    ...
    const minOrderCents =
      (currentRestaurant as { min_order_cents?: number | null } | null)
        ?.min_order_cents ?? 0;
    if (minOrderCents > 0 && cartTotal * 100 < minOrderCents) {
      toast.error(`Minimum order ${fmtPrice(minOrderCents / 100)}`);
      return;
    }
    const result = await placeOrder({
      ...
      items: cart.map((c) => ({
        ...c,
        specialInstructions: specialInstructions[c.menuItemId] || undefined,
      })),
      orderMode,
      deliveryAddress:
        orderMode === "pickup"
          ? currentRestaurant?.address || "Pickup at restaurant"
          : deliveryAddress,
      ...
      specialInstructions:
        [
          ...
          scheduleMode === "later" && scheduleTime
            ? `[Scheduled: ${new Date(scheduleTime).toLocaleString()}]`
            : null,
          orderMode === "delivery" && contactlessDelivery
            ? "[Contactless]"
            : null,
          noUtensils ? "[No utensils]" : null,
          orderMode === "delivery" ? deliveryInstructions || null : null,
        ]
          .filter(Boolean)
          .join(" ") || undefined,
      ...
      scheduledFor:
        scheduleMode === "later" && scheduleTime
          ...
          : undefined,
      isExpress: orderMode === "delivery" && selectedSpeed === "priority",
      expressFee: checkoutSpeedExtra,
    ...
    if (result) {
      // Once a durable order exists, remove the resubmittable cart for every
      // payment outcome. Any next action continues that exact order ID.
      setCart([]);
      try {
        localStorage.removeItem(CART_STORAGE_KEY);
      } catch {}

      if (result.outcome === "card_payment_required") {
        setTrackedOrderId(null);
        setCardPaymentSession(result);
        return;
      }

      if (result.outcome === "external_checkout_started") {
        // The browser is leaving for the provider. PayPal/Square return to the
        // saved order flow; no local success is shown before provider proof.
        return;
      }

      if (result.outcome === "confirmation_pending") {
        setTrackedOrderId(null);
        navigate(`/eats/track/${result.orderId}`, { replace: true });
        return;
      }
      ...
      if (result.outcome === "payment_failed") {
        ...
        toast.error("Payment was not completed", {
          description:
            "The saved order was not dispatched. You can review or cancel it safely.",
        });
        navigate(`/eats/track/${result.orderId}`, { replace: true });
        return;
      }
      ...
      toast.success("Order placed!", {
        ...
        action: {
          label: "Track",
          onClick: () => navigate(`/eats/track/${result.orderId}`),
        },
        duration: 6000,
      });
    }
  ...
  const handleCardPaymentSuccess = async () => {
    if (!cardPaymentSession) return;
    const orderId = cardPaymentSession.orderId;
    setCardPaymentSession(null);
    toast.success("Payment submitted", {
      description:
        "Opening your saved order while the payment webhook confirms it.",
    });
    navigate(`/eats/track/${orderId}`, { replace: true });
  };

  const handleCardPaymentCancel = async () => {
    if (!cardPaymentSession || cancellingCardPayment) return;
    setCancellingCardPayment(true);
    try {
      const { data, error } = await supabase.functions.invoke(
        "cancel-eats-order",
        {
          body: {
            order_id: cardPaymentSession.orderId,
            reason: "payment_cancelled",
          },
        },
      );
      if (error || data?.ok !== true || data?.status !== "cancelled") {
        throw error || new Error(data?.error || "Cancellation not confirmed");
      }
      const orderId = cardPaymentSession.orderId;
      setCardPaymentSession(null);
      toast.info("Payment cancelled. No new order was created.");
      navigate(`/eats/track/${orderId}`, { replace: true });
    } catch (error) {
      console.error("[EatsLanding] card payment cancellation failed", error);
      toast.error(
        "Could not confirm cancellation. This saved order remains open; no second order was created.",
      );
    } finally {
      setCancellingCardPayment(false);
    }
  };
  ...
  const handleCancelTrackedOrder = async () => {
    ...
    try {
      ...
      if (
        ...
        throw (
          error ||
          new Error((data as any)?.error || "Cancellation was not confirmed")
        );
      }
      const r = data as {
        refund_cents?: number;
          ...
          {inCart ? (
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="w-8 h-8 rounded-full bg-muted flex items-center justify-center touch-manipulation active:scale-90 hover:bg-muted/80 transition-colors"
              >
              ...
              <button
                ...
                aria-label="Increase"
                onClick={() => updateQuantity(item.id, 1)}
                className="w-8 h-8 rounded-full bg-ig-gradient text-white flex items-center justify-center touch-manipulation active:scale-90 hover:shadow-md hover:shadow-primary/30 transition-shadow"
              >
            ...
            <Button
              ...
              className="rounded-xl h-9 px-4 gap-1.5 font-bold text-xs border-primary/30 text-primary hover:bg-primary/10 hover:border-primary/50 hover:shadow-sm hover:shadow-primary/20 shrink-0 transition-all"
            >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-orange-500/[0.02] relative">
          ...
          >
            {showCartRestore && cart.length > 0 && !trackedOrderId && (
              ...
              >
                ...
                <span>
                  Welcome back — {cartCount} item{cartCount !== 1 ? "s" : ""}{" "}
                  saved in your cart
                </span>
                <button
                  type="button"
                ...
                <button
                  type="button"
                  onClick={() => setShowCartRestore(false)}
                  aria-label="Dismiss"
                    ...
                    >
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter") persistSearch(searchQuery);
                          }}
                          onBlur={() => persistSearch(searchQuery)}
                          className={cn(
                            "pl-10 h-12 rounded-xl bg-card border-border/50 shadow-sm",
                        ...
                        <button
                          ...
                          className={cn(
                            "absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg flex items-center justify-center transition-all touch-manipulation active:scale-90",
                            voiceListening
                      ...
                      {!searchQuery && recentSearches.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70 self-center">
                            Recent:
                          </span>
            ...
            <section className="pt-2 pb-8">
              <div className="container mx-auto px-4 max-w-6xl">
                    ...
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                      {recentOrders.map((o) => (
                        <button
                          ...
                          key={o.store_id}
                          onClick={() => {
                            trackRecentlyViewed(o.store_id);
                            setSelectedRestaurantId(o.store_id);
                            setStep("restaurant");
                          }}
                ...
                {(() => {
                  if (loadingRestaurants || recentOrders.length === 0)
                    return null;
                  const likedCuisines = new Set(
                    recentOrders
                      .map((o) => (o.cuisine || "").toLowerCase())
                      .filter(Boolean),
                  );
                  if (!likedCuisines.size) return null;
                  const recentIds = new Set(
                    recentOrders.map((o) => o.store_id),
                  );
                  const recs = restaurants
                    .filter(
                      (r) =>
                        !recentIds.has(r.id) &&
                        likedCuisines.has((r.cuisine_type || "").toLowerCase()),
                    )
                    .slice(0, 6);
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-violet-500 font-bold">
                                {canonicalCuisine(r.cuisine_type)}
                              </p>
                            </div>
                          </button>
                        ))}
                      ...
                      <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 snap-x snap-mandatory overscroll-x-contain">
                        {[...restaurants]
                          ...
                          .map((r) => (
                            ...
                            >
                              <div
                                className="w-8 h-8 rounded-lg bg-gradient-to-br from-orange-500/15 to-rose-500/10 flex items-center justify-center text-base leading-none shrink-0"
                                aria-hidden
                              >
                ...
                {recentlyViewedIds.length > 0 &&
                  (() => {
                    const recents = recentlyViewedIds
                      ...
                      .filter((r): r is (typeof restaurants)[number] =>
                        Boolean(r),
                      )
                      .slice(0, 6);
                ...
                >
                  <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 mb-1 snap-x snap-mandatory overscroll-x-contain">
                    <button
                      ...
                      onClick={() => {
                        setPromoCode("ZIVO10");
                        toast.success("Code ZIVO10 ready — apply at checkout");
                      }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-bold">
                          On orders over {fmtPrice(20)} today
                        </p>
                      </div>
                    </button>
                    <button
                ...
                {(activeCategory !== "All" ||
                  ...
                  <div className="sticky top-0 z-20 -mx-4 px-4 py-2 bg-background/90 backdrop-blur-md border-b border-border/30 flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    {searchQuery && (
                      <span className="px-2 py-0.5 rounded-full bg-muted text-foreground text-[11px] font-bold">
                        "{searchQuery}"
                      </span>
                    )}
                    <button
                ...
                <div className="flex items-center mb-3">
                  <div className="flex bg-muted/40 rounded-xl p-1 ml-auto">
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all touch-manipulation active:scale-95",
                        viewMode === "list"
                          ? "bg-background text-foreground shadow-sm"
                          : "text-muted-foreground",
                      )}
                ...
                <div className="flex gap-2 mb-6 flex-wrap">
                  {(["recommended", "rating", "time", "nearby"] as const).map(
                    (s) => {
                      const Icon =
                        s === "recommended"
                          ? Flame
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            setSortBy(s);
                            if (s === "nearby" && !userCoords)
                              requestLocation();
                          }}
                          className={cn(
                            "flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all touch-manipulation active:scale-95",
                            sortBy === s
                  ...
                  <div className="relative rounded-2xl overflow-hidden border border-border/40 mb-5 h-[420px] bg-gradient-to-br from-emerald-500/5 via-primary/5 to-orange-500/5">
                    ...
                    {filtered.slice(0, 12).map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className="px-2 py-1 rounded-lg bg-background shadow-lg border border-border/40 text-[10px] font-bold text-foreground whitespace-nowrap mb-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            {r.name}
                          </div>
                      ...
                      >
                        <div className="group relative rounded-2xl bg-card border border-border/40 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/8 transition-all duration-200">
                          <button
                            type="button"
                            onClick={() => {
                              trackRecentlyViewed(restaurant.id);
                              setSelectedRestaurantId(restaurant.id);
                              setStep("restaurant");
                            }}
                            className="flex items-center gap-3 w-full text-left px-3 py-3 touch-manipulation active:scale-[0.99]"
                          >
                            {/* Thumbnail */}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 mb-0.5">
                                ...
                                {restaurant.is_open &&
                                  ((restaurant.rating_count ?? 0) > 5 ||
                                    (restaurant.rating ?? 0) >= 4.5) && (
                                    <Flame className="w-3 h-3 text-rose-500 shrink-0" />
                                  )}
                                {restaurant.delivery_fee_cents === 0 && (
                                  <span className="shrink-0 text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-600">
                                    Free
                                  </span>
                                )}
                              </div>
                              <div className="flex items-center gap-2 text-[11px] text-muted-foreground flex-wrap">
                                <span className="truncate max-w-[80px]">
                                  {canonicalCuisine(restaurant.cuisine_type)}
                                </span>
                                ...
                                <span className="flex items-center gap-0.5 shrink-0">
                                  ...
                                  {restaurant.avg_prep_time ?? 25}–
                                  {(restaurant.avg_prep_time ?? 25) + 15}m
                                </span>
                                <span className="text-border">·</span>
                                <span className="flex items-center gap-0.5 shrink-0">
                                  ...
                                  {restaurant.delivery_fee_cents === 0 ? (
                                    ...
                                    fmtPrice(
                                      (restaurant.delivery_fee_cents ?? 399) /
                                        100,
                                    )
                                  )}
                                </span>
                                    ...
                                    <>
                                      ...
                                      <span className="shrink-0">
                                        {haversineKm(userCoords, {
                                          lat: (restaurant as { lat: number })
                                            .lat,
                                          lng: (restaurant as { lng: number })
                                            .lng,
                                        }).toFixed(1)}{" "}
                                        km
                                      </span>
                          ...
                          <div className="absolute top-1/2 -translate-y-1/2 right-8 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none group-hover:pointer-events-auto">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/eats/reserve?restaurantId=${restaurant.id}&restaurantName=${encodeURIComponent(restaurant.name)}`,
                                );
                              }}
                      ...
                      <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5">
                        <div className="flex-1">
                          ...
                          <h3 className="text-xl sm:text-2xl font-bold mb-1">
                            Grow your restaurant with ZIVO
                          </h3>
                          <p className="text-sm opacity-90 max-w-md">
                            Reach new customers, manage orders in one place, and
                            get paid weekly with zero setup fees.
                          </p>
                        </div>
                        <button
                          type="button"
              ...
              >
                <div className="flex -space-x-2">
                  {cart.slice(0, 3).map((it, idx) => (
                    <div
                      key={it.menuItemId}
                      className="w-8 h-8 rounded-full bg-white/30 ring-2 ring-primary overflow-hidden flex items-center justify-center"
                      style={{ zIndex: 3 - idx }}
                    >
                      {it.imageUrl ? (
                        <img
                  ...
                  {cart.length > 3 && (
                    <div className="w-8 h-8 rounded-full bg-white/20 ring-2 ring-primary flex items-center justify-center text-[10px] font-bold">
                      +{cart.length - 3}
                    </div>
                  )}
                </div>
                ...
                <motion.span
                  ...
                  animate={{ scale: 1 }}
                  transition={{ type: "spring", stiffness: 500, damping: 18 }}
                  className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-white/20 text-[11px] tabular-nums"
                >
                  {cartCount}
                </motion.span>
                <motion.span
            ...
            {showNotifPrompt && cartCount === 0 && (
              <div
                className="fixed left-1/2 -translate-x-1/2 bottom-6 z-40 w-[min(560px,calc(100%-2rem))] rounded-2xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white shadow-xl shadow-violet-500/30 px-4 py-3 flex items-center gap-3"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] opacity-90">
                    Know exactly when your food arrives
                  </p>
                </div>
                <button
                  type="button"
            ...
            {pwaPrompt && !pwaDismissed && (
              <div
                className="fixed left-1/2 -translate-x-1/2 bottom-6 z-40 w-[min(560px,calc(100%-2rem))] rounded-2xl bg-foreground text-background shadow-xl shadow-foreground/30 px-4 py-3 flex items-center gap-3"
                style={{
                  bottom: `calc(var(--zivo-safe-bottom,0px) + ${cartCount > 0 ? 96 : 24}px)`,
                }}
              >
                <Sparkles className="w-4 h-4 shrink-0" />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] opacity-80">
                    Faster checkout, save items, push updates
                  </p>
                </div>
                <button
                  type="button"
            ...
            <div
              className="fixed right-4 z-30 group flex flex-col items-end"
              style={{
                bottom: `calc(var(--zivo-safe-bottom,0px) + ${cartCount > 0 ? 96 : 24}px)`,
              }}
            >
            ...
            <AnimatePresence>
              {showBackToTop && (
                <motion.button
                  ...
                  style={{
                    bottom: `calc(var(--zivo-safe-bottom,0px) + ${cartCount > 0 ? 156 : 84}px)`,
                  }}
          ...
          >
            <div className="sticky top-0 safe-area-top z-20 bg-background/95 backdrop-blur-2xl border-b border-border/30">
              <div className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <h1 className="text-base font-bold text-ig-gradient">
                    {currentRestaurant.name}
                  </h1>
                  <p className="text-[10px] text-muted-foreground">
                    {canonicalCuisine(currentRestaurant.cuisine_type)} ·{" "}
                    {currentRestaurant.avg_prep_time ?? 25}-
                    {(currentRestaurant.avg_prep_time ?? 25) + 15} min
                  </p>
                </div>
                <button
                  type="button"
                ...
                <button
                  ...
                  onClick={() =>
                    openShareToChat({
                      ...
                      meta:
                        (currentRestaurant as { delivery_fee?: number | null })
                          .delivery_fee != null
                          ? (
                              currentRestaurant as {
                                delivery_fee?: number | null;
                              }
                            ).delivery_fee === 0
                            ? "Free delivery"
                            : `$${(currentRestaurant as { delivery_fee?: number | null }).delivery_fee} delivery`
                          : undefined,
                      deepLink: "/eats",
                ...
                (currentRestaurant.rating_count ?? 0) > 0 ? (
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600">
                    New
                  </span>
                )}
                <span
            ...
            <div className="relative h-48 overflow-hidden bg-muted/20">
              {currentRestaurant.cover_image_url ? (
                <img
                  src={currentRestaurant.cover_image_url}
                ...
                <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-orange-500/20 via-amber-500/15 to-rose-500/10 relative">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-foreground/60 relative">
                    {canonicalCuisine(currentRestaurant.cuisine_type)}
                  </span>
                </div>
              )}
              ...
              <div className="absolute bottom-4 left-4 flex items-center gap-2">
                {currentRestaurant.delivery_fee_cents === 0 && (
                  <Badge className="bg-ig-gradient text-white text-[10px] font-bold gap-1">
                    <Truck className="w-3 h-3" /> Free Delivery
                  </Badge>
                )}
                <Badge
                  variant="outline"
                  className="bg-card/80 backdrop-blur text-[10px] font-bold gap-1"
                >
                  <Timer className="w-3 h-3" />{" "}
                  {currentRestaurant.avg_prep_time ?? 25}m prep
                </Badge>
              </div>
            </div>
              ...
              <div className="px-4 pt-4 max-w-lg mx-auto">
                <button
                  ...
                  onClick={() =>
                    navigate(
                      zivoRouteUrl({
                        lat: (currentRestaurant as { latitude?: number | null })
                          .latitude,
                        lng: (
                          currentRestaurant as { longitude?: number | null }
                        ).longitude,
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {(currentRestaurant.rating_count ?? 0) > 0 && (
                    <span className="text-[11px] font-bold text-muted-foreground">
                      {currentRestaurant.rating_count} reviews
                    </span>
                  )}
                </div>
                {(currentRestaurant.rating_count ?? 0) > 0 ? (
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <div className="flex gap-0.5 mb-1">
                          {[1, 2, 3, 4, 5].map((i) => (
                            <Star
                              ...
                              className={cn(
                                "w-3.5 h-3.5",
                                i <= Math.round(currentRestaurant.rating ?? 0)
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          Based on {currentRestaurant.rating_count} customer
                          reviews
                        </p>
                      </div>
                    </div>
                  ...
                  <p className="text-xs text-muted-foreground py-2">
                    No reviews yet — be the first to order and rate.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="px-4 py-6 max-w-lg mx-auto space-y-3">
              ...
              {loadingMenu && (
                <div className="space-y-3">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div
                      key={i}
                      className="p-4 rounded-2xl bg-card border border-border/40 flex items-center gap-4"
                    >
              ...
              {!loadingMenu && menuItems.length === 0 && (
                <div className="text-center py-12 px-6 rounded-2xl border border-dashed border-border/60 bg-muted/20">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    {currentRestaurant.name} hasn't published their menu yet.
                  </p>
                  <button
                    type="button"
                    onClick={handleBack}
                              ...
                              {inCart ? (
                                <div className="flex items-center gap-1">
                                  ...
                                  <button
                                    ...
                                    aria-label="Increase"
                                    onClick={() => updateQuantity(item.id, 1)}
                                    className="w-6 h-6 rounded-full bg-ig-gradient text-white flex items-center justify-center active:scale-90"
                                  >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    addToCart(item, currentRestaurant!.id)
                                  }
                                  aria-label="Add"
              ...
              {(() => {
                ...
                return ordered.map(([cat, items]) => (
                  <div key={cat} className="space-y-3">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground pt-2 sticky top-[88px] bg-background/95 backdrop-blur-sm py-2 z-10 -mx-4 px-4 border-b border-border/20">
                      {cat}
                    </h3>
            ...
            {cartCount > 0 && (
              <div className="fixed bottom-0 left-0 right-0 z-40 p-4 bg-background/95 backdrop-blur-2xl border-t border-border/30 safe-area-bottom">
                ...
                >
                  ...
                  items <span className="ml-auto">{fmtPrice(cartTotal)}</span>
                </Button>
              </div>
            )}
          </motion.div>
          ...
          >
            ...
            {cart.length === 0 ? (
              ...
              <div className="px-4 pb-8 max-w-lg mx-auto">
                ...
                <div className="text-center mb-8">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Pick a restaurant and add your favorites
                  </p>
                </div>
                <motion.button
                  whileTap={{ scale: 0.96 }}
              ...
              <div className="px-4 pt-4 pb-4 max-w-lg mx-auto space-y-3">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <span className="text-[11px] font-bold text-primary">
                        {(currentRestaurant.avg_prep_time ?? 25) + 10}–
                        {(currentRestaurant.avg_prep_time ?? 25) + 20} min
                      </span>
                    </div>
                  </motion.div>
                )}
                ...
                <div className="rounded-2xl bg-card border border-border/40 p-3 flex items-center gap-3">
                  <button
                    ...
                    className={cn(
                      ...
                      noUtensils ? "bg-emerald-500" : "bg-muted/60",
                    )}
                  >
                    <span
                      className={cn(
                        "absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all",
                        noUtensils ? "left-[18px]" : "left-0.5",
                      )}
                    />
                  </button>
                  <div className="flex-1">
                    <p className="text-xs font-extrabold text-foreground flex items-center gap-1.5">
                      <Leaf className="w-3.5 h-3.5 text-emerald-500" /> Skip
                      utensils
                    </p>
                    <p className="text-[10px] text-muted-foreground">
                ...
                {(() => {
                  const threshold = 20;
                  const remaining = Math.max(0, threshold - cartTotal);
                  if (remaining === 0 || remaining > 8 || cart.length === 0)
                    return null;
                  const popularExtras = menuItems
                    .filter(
                      (m) =>
                        m.price <= remaining + 1 &&
                        !cart.some((c) => c.menuItemId === m.id),
                    )
                    .slice(0, 3);
                  ...
                  return (
                    <div className="rounded-2xl bg-amber-500/5 border border-amber-500/25 p-3 space-y-2">
                      <p className="text-xs font-extrabold text-amber-600 flex items-center gap-1.5">
                        <Sparkles className="w-3.5 h-3.5" /> Add{" "}
                        {fmtPrice(remaining)} more for free delivery
                      </p>
                      <div className="flex gap-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-1 -mx-1 px-1 overscroll-x-contain">
                        {popularExtras.map((m) => (
                          ...
                          >
                            ...
                            <div className="text-left">
                              <p className="text-xs font-extrabold text-foreground truncate max-w-[100px]">
                                {m.name}
                              </p>
                ...
                <div className="rounded-2xl bg-card border border-border/40 overflow-hidden">
                  ...
                  <div className="p-4 space-y-2.5 text-sm">
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-border/20">
                      ...
                      <motion.span
                        key={cartTotal + checkoutDeliveryFee + serviceFee}
                        initial={{ scale: 1.1 }}
                        animate={{ scale: 1 }}
                        transition={{ type: "spring", stiffness: 400 }}
                        className="font-black text-xl text-primary"
                      >
                        {fmtPrice(cartTotal + checkoutDeliveryFee + serviceFee)}
                      </motion.span>
                    </div>
                  </div>
                </div>
            ...
            {cart.length > 0 &&
              (() => {
                ...
                const checkoutDisabled = belowMin || !EATS_ORDERING_ENABLED;
                return (
                  <div className="fixed bottom-0 left-0 right-0 z-40 px-4 pt-3 pb-[calc(env(safe-area-inset-bottom,0px)+72px)] bg-background/95 backdrop-blur-2xl border-t border-border/20">
                    {!EATS_ORDERING_ENABLED && (
                      <p
                        role="status"
                        className="mb-2 text-center text-xs font-bold text-amber-700 dark:text-amber-300"
                      >
                        Menus are available to browse. Ordering opens after
                        restaurant delivery locations are verified.
                      </p>
                    )}
                    {belowMin && (
                      <p className="text-center text-xs font-extrabold text-amber-600 mb-2 flex items-center justify-center gap-1.5">
                        <Sparkles className="w-3.5 h-3.5" /> Add{" "}
                        {fmtPrice(minOrder - cartTotal)} more (min{" "}
                        {fmtPrice(minOrder)})
                      </p>
                    )}
                    <motion.button
                      ...
                      onClick={() =>
                        !EATS_ORDERING_ENABLED
                          ? toast.error(
                              "Food ordering is temporarily unavailable",
                            )
                          : belowMin
                            ? toast.error(
                                `Minimum order is $${minOrder.toFixed(2)}`,
                              )
                            : setStep("checkout")
                      }
                      disabled={checkoutDisabled}
                      className={cn(
                        "w-full h-14 rounded-2xl font-extrabold text-base flex items-center justify-between px-5 shadow-lg",
                        checkoutDisabled
            ...
            <div className="px-4 py-6 max-w-lg mx-auto space-y-5">
              ...
              {orderMode === "delivery" ? (
                <div className="rounded-2xl bg-card border border-border/40 p-4 space-y-3">
                    ...
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide -mx-1 px-1 pb-1">
                      {savedAddresses.map((a) => (
                        <button
                          type="button"
                          key={a}
                          onClick={() => setDeliveryAddress(a)}
                          className={cn(
                            "shrink-0 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all touch-manipulation active:scale-95 max-w-[200px] truncate",
                            deliveryAddress === a
                  ...
                  />
                  <Input
                    placeholder="Delivery instructions (e.g., buzz #204)"
                  ...
                  <div className="flex items-center gap-3 pt-1">
                    <button
                      type="button"
                      onClick={() =>
                        setContactlessDelivery(!contactlessDelivery)
                      }
                      className={cn(
                        "w-10 h-6 rounded-full transition-all relative shrink-0",
                        contactlessDelivery ? "bg-primary" : "bg-muted/60",
                      )}
                    >
                      <span
                        className={cn(
                          "absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all",
                          contactlessDelivery ? "left-[18px]" : "left-0.5",
                        )}
                      />
                    </button>
                ...
                <div className="rounded-2xl bg-card border border-border/40 p-4 space-y-2">
                  <h3 className="font-bold text-sm flex items-center gap-2">
                    <Store className="w-4 h-4 text-primary" /> Pickup at
                    restaurant
                  </h3>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {currentRestaurant?.address ??
                      "Pickup address will be confirmed with your order."}
                  </p>
                </div>
              )}
              ...
              <div className="rounded-2xl bg-card border border-border/40 p-4">
                <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <CalendarCheck className="w-3 h-3" /> When
                </h3>
                <div className="flex bg-muted/40 rounded-xl p-1 mb-2">
                  <button
                    ...
                    className={cn(
                      "flex-1 py-2 rounded-lg text-xs font-bold transition-all touch-manipulation active:scale-95",
                      scheduleMode === "now"
                        ? "bg-background text-foreground shadow-sm"
                        : "text-muted-foreground",
                    )}
                ...
                {scheduleMode === "later" &&
                  (() => {
                    ...
                    const next = new Date(
                      Math.ceil(
                        (now.getTime() + 60 * 60 * 1000) / (30 * 60 * 1000),
                      ) *
                        30 *
                        60 *
                        1000,
                        ...
                        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                          {slots.map((s) => (
                            <button
                              ...
                              className={cn(
                                "shrink-0 px-3 py-2 rounded-xl text-xs font-bold transition-all touch-manipulation active:scale-95",
                                scheduleTime === s.iso
                  ...
                  <div className="space-y-2">
                    {deliverySpeedOptions.map((opt) => (
                      <button
                        ...
                        className={cn(
                          "w-full flex items-center justify-between p-3 rounded-xl transition-all touch-manipulation active:scale-[0.98]",
              ...
              <div className="rounded-2xl bg-card border border-border/40 p-4">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {[
                    ...
                    {
                      id: "wallet" as const,
                      label: fmtPrice(walletBalanceCents / 100),
                      Icon: Wallet,
                    },
                    {
                      id: "paypal" as const,
                    ...
                    .filter((p) => isEatsPaymentRailEnabled(p.id))
                    .map((p) => (
                      <button
                        type="button"
                        key={p.id}
                        onClick={() => {
                          if (
                            p.id === "wallet" &&
                            walletBalanceCents < Math.round(grandTotal * 100)
                          ) {
                            toast.error(
                              `Insufficient wallet balance ($${(walletBalanceCents / 100).toFixed(2)})`,
                            );
                            return;
                          }
                          setPaymentType(p.id);
                        ...
                        className={cn(
                          "flex flex-col items-center justify-center gap-1 py-3 rounded-xl text-xs font-bold transition-all touch-manipulation active:scale-95",
                          paymentType === p.id
                            ? "bg-ig-gradient text-white shadow-md"
                ...
                {(paymentType === "paypal" || paymentType === "square") && (
                  <p className="text-[11px] text-muted-foreground mt-2">
                    ...
                    {paymentType === "paypal" ? "PayPal" : "Square"} to confirm
                    payment, then come right back to track your order.
                  </p>
                )}
                {paymentType === "card" && (
                  <p className="mt-2 text-[11px] text-muted-foreground">
                    Stripe will show card and any eligible Apple Pay or Google
                    Pay option in the secure payment step.
                  </p>
                )}
              </div>
                  ...
                  <div className="flex gap-2">
                    {tipOptions.map((t) => {
                      const tipPreviewAmt =
                        t.id === "custom"
                          ? null
                          : Math.round(cartTotal * t.pct * 100) / 100;
                      return (
                        <button
                          type="button"
                          key={t.id}
                          ...
                          className={cn(
                            "flex-1 py-2 rounded-xl text-xs font-bold transition-all touch-manipulation active:scale-95 flex flex-col items-center gap-0.5",
                            selectedTip === t.id
                              ? "bg-ig-gradient text-white shadow-md"
                              : "bg-muted/50 text-muted-foreground border border-border/40",
                          )}
              ...
              >
                <h3
                  className={cn(
                    "text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5",
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        const { data: promo } = (await (supabase as any)
                          .from("promo_codes")
                          .select(
                            "id, discount_type, discount_value, is_active, min_fare, expires_at",
                          )
                        ...
                        if (promo.min_fare && cartTotal < promo.min_fare) {
                          toast.error(
                            `Minimum order $${Number(promo.min_fare).toFixed(2)} required`,
                          );
                          return;
                        }
                        ...
                        setPromoData(
                          promo.discount_type === "percent"
                            ? {
                                discount_percent: Number(promo.discount_value),
                                discount_amount_cents: null,
                              }
                            : {
                                discount_percent: null,
                                discount_amount_cents: Math.round(
              ...
              <div className="rounded-2xl bg-card border border-border/40 p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Service fee</span>
                  <span className="font-bold">{fmtPrice(serviceFee)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Tax (10%)</span>
                  <span className="font-bold">{fmtPrice(taxAmount)}</span>
                </div>
                {checkoutSpeedExtra > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Priority fee</span>
                    <span className="font-bold">
                      {fmtPrice(checkoutSpeedExtra)}
                    </span>
                  </div>
                )}
                {checkoutTipAmount > 0 && (
              ...
              <div className="rounded-2xl bg-primary/5 border border-primary/20 p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-foreground">
                    ...
                    :{" "}
                    {currentRestaurant
                      ? `${(currentRestaurant.avg_prep_time ?? 25) + (orderMode === "pickup" ? 0 : 10)}-${(currentRestaurant.avg_prep_time ?? 25) + (orderMode === "pickup" ? 10 : 20)} min`
                      : "25-35 min"}
                  </p>
                  <p className="text-xs text-muted-foreground">
              ...
              {(() => {
                ...
                if (orderMode === "delivery" && !deliveryAddress.trim())
                  blockers.push("Enter delivery address");
                ...
                if (
                  ...
                  blockers.push("Pick a future time");
                if (minOrderCents > 0 && cartTotal * 100 < minOrderCents)
                  blockers.push(`Min ${fmtPrice(minOrderCents / 100)}`);
                if (
                  paymentType === "wallet" &&
                  walletBalanceCents < Math.round(grandTotal * 100)
                )
                  blockers.push("Wallet balance too low");
                ...
                return (
                  <>
                    {blockers.length > 0 && (
                      <ul className="space-y-1 mb-2">
                        {blockers.map((b) => (
                          <li
                            key={b}
                            className="text-[11px] font-medium text-amber-600 flex items-center gap-1.5"
                          >
                    ...
                    <Button
                      ...
                      disabled={blocked}
                      className={cn(
                        "w-full h-14 text-base font-bold gap-3 rounded-2xl text-primary-foreground shadow-lg active:scale-[0.98] transition-all",
                        blocked
                          ? "bg-muted text-muted-foreground cursor-not-allowed"
                          : "bg-gradient-to-r from-primary to-emerald-500 shadow-primary/25",
                      )}
                    >
                      {placingOrder ? (
      ...
      {/* Card/Apple Pay/Google Pay must complete inside the saved order. The
          checkout beneath this sheet is intentionally not resubmittable. */}
      <AnimatePresence>
        {cardPaymentSession && (
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="eats-secure-payment-title"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[70] overflow-y-auto bg-background/98 px-4 py-[max(1rem,env(safe-area-inset-top))] backdrop-blur-xl"
          >
            <div className="mx-auto flex min-h-full w-full max-w-lg items-center py-6">
              <div className="w-full rounded-3xl border border-border/40 bg-card p-5 shadow-2xl">
                <div className="mb-5">
                  <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary">
                    Saved order #{cardPaymentSession.trackingCode}
                  </p>
                  <h2
                    id="eats-secure-payment-title"
                    className="mt-1 text-xl font-black text-foreground"
                  >
                    Complete secure payment
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Your order is saved once. It will dispatch only after Stripe
                    confirms payment.
                  </p>
                </div>
                <div
                  aria-busy={cancellingCardPayment}
                  className={cn(
                    "transition-opacity",
                    cancellingCardPayment && "pointer-events-none opacity-60",
                  )}
                >
                  <EatsInlinePaymentForm
                    clientSecret={cardPaymentSession.clientSecret}
                    amountCents={cardPaymentSession.amountCents}
                    returnUrl={`${window.location.origin}/eats/track/${cardPaymentSession.orderId}`}
                    onCancel={() => void handleCardPaymentCancel()}
                    onSuccess={() => handleCardPaymentSuccess()}
                  />
                </div>
                {cancellingCardPayment && (
                  <p
      ...
      <AnimatePresence>
        {placingOrder && (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              Confirming with the restaurant
            </p>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {trackedOrderId && (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-8 text-center">
              {statusStep === 0 &&
                "Your order is on its way to the restaurant."}
            ...
            <div className="flex flex-col items-center gap-3 w-full max-w-xs">
              <button
                ...
                onClick={() => {
                  setTrackedOrderId(null);
                  navigate(`/eats/track/${trackedOrderId}`);
                }}
