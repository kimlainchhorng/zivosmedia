...
export default function EatsDriverDeliveryPage() {
  ...
  const [tab, setTab] = useState<"available" | "active" | "completed">(
    "available",
  );
  ...
  const loadAllData = useCallback(async () => {
    ...
    const [offersRes, activeRes, completedRes] = await Promise.all([
      ...
      supabase
        ...
        .eq("assigned_driver_id", user.id)
        .eq("job_type", "food_delivery" as any)
        .in("status", ["completed", "canceled"] as any[])
        .order("created_at", { ascending: false })
        .limit(20),
    ]);
    ...
    if (offerData.length > 0) {
      ...
      const jobMap = Object.fromEntries(
        (jobsData || []).map((j: any) => [j.id, j]),
      );
      setOffers(
        offerData
          .filter((offer) => Boolean(jobMap[offer.job_id]))
          .map((offer) => ({ ...offer, job: jobMap[offer.job_id] })),
      );
    } else {
      setOffers([]);
    }
  ...
  useEffect(() => {
    if (!driverId || !user) return;
    void loadAllData();
    const interval = setInterval(() => void loadAllData(), 15000);
  ...
  }, [driverId, loadAllData, user]);

  async function acceptOffer(offerId: string) {
    setAccepting(offerId);
    try {
      const { data, error } = await (supabase as any).rpc(
        "accept_eats_job_offer",
        {
          p_offer_id: offerId,
        },
      );
      ...
      if (error || !result?.ok)
        throw (
          error ?? new Error(result?.code || "Offer is no longer available")
        );
  ...
  async function declineOffer(offerId: string) {
    const { error } = await supabase
      ...
      .eq("id", offerId);
    if (error) {
      toast.error("Could not decline this offer. Refresh and try again.");
      return;
    }
  ...
  async function updateJobStatus(jobId: string, newStatus: string) {
    ...
    const foodOrderId =
      ...
      job?.notes?.match(/Food order:\s*(.+)/)?.[1]?.trim();
    if (!foodOrderId) {
      toast.error(
        "This delivery is missing its food order link. Contact support.",
      );
      setUpdatingJob(null);
      return;
    }
    ...
    if (error || (data as { error?: string } | null)?.error) {
      console.error(
        "[EatsDriverDelivery] eats-order-state-update (driver_status) failed",
        error ?? data,
      );
      toast.error("Status was not changed. Refresh and try again.");
    } else {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4 p-6">
        ...
        <p className="text-muted-foreground text-center text-sm">
          Sign up as a ZIVO driver to start delivering food.
        </p>
        <Button onClick={() => navigate("/drive")} variant="outline">
          Become a Driver
        </Button>
      </div>
    );
  ...
  const statusFlow: Record<
    string,
    { label: string; next?: { label: string; status: string } }
  > = {
    assigned: {
      label: "Assigned",
      next: { label: "Head to Pickup", status: "en_route_pickup" },
    },
    enroute_pickup: {
      label: "En Route to Pickup",
      next: { label: "Arrived at Restaurant", status: "arrived_pickup" },
    },
    arrived_pickup: {
      label: "At Restaurant",
      next: { label: "Picked Up — Delivering", status: "en_route_dropoff" },
    },
    enroute_dropoff: {
      label: "Delivering",
      next: { label: "Delivered ✓", status: "completed" },
    },
  };
      ...
      <div className="sticky top-0 safe-area-top z-30 bg-background/95 backdrop-blur-md border-b border-border/40">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-ig-gradient">
              Food Deliveries
            </h1>
                  ...
                  offers.map((offer) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <Badge
                          variant="outline"
                          className="text-[10px] bg-amber-500/10 text-amber-600 border-amber-500/20 rounded-full"
                        >
                          New Request
                        </Badge>
                        <p className="text-sm font-bold text-foreground">
                          ${(offer.job?.price_total || 0).toFixed(2)}
                        </p>
                      </div>
                      <div className="space-y-2 text-xs">
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-foreground">
                            {offer.job?.pickup_address || "Restaurant"}
                          </p>
                        </div>
                        <div className="flex items-start gap-2">
                          <div className="w-2 h-2 rounded-full bg-primary mt-1 shrink-0" />
                          <p className="text-foreground">
                            {offer.job?.dropoff_address || "Customer"}
                          </p>
                        </div>
                      </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <Badge
                            variant="outline"
                            className="text-[10px] bg-primary/10 text-primary border-primary/20 rounded-full"
                          >
                            {flow?.label || job.status}
                          </Badge>
                          <p className="text-sm font-bold text-foreground">
                            ${(job.price_total || 0).toFixed(2)}
                          </p>
                        </div>
                        ...
                        {job.notes && (
                          <p className="text-xs text-muted-foreground bg-muted/30 px-2 py-1 rounded-lg">
                            {job.notes}
                          </p>
                        )}
                        ...
                        {flow?.next && (
                          <Button
                            ...
                            onClick={() =>
                              updateJobStatus(job.id, flow.next!.status)
                            }
                          >
                            {updatingJob === job.id ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
