...
import {
  Coffee, Loader2, AlertCircle, Plus, Minus, ShoppingBag, ArrowRight, ChevronLeft, X,
} from "lucide-react";
...
export default function PublicCafeOrderPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [catRes, itemRes, grpRes, modRes, linkRes, tableRes, loyRes, statusRes, settingsRes, popularRes, reviewRes] = await Promise.all([
        supabase.from("cafe_categories" as never).select("id,name,sort_order").eq("store_id", s.id).eq("is_active", true).order("sort_order"),
        supabase.from("cafe_menu_items" as never).select("id,name,description,price_cents,image_url,category_id,is_featured,is_sold_out,is_vegetarian,is_vegan,is_gluten_free").eq("store_id", s.id).eq("is_active", true).order("sort_order"),
        ...
        supabase.from("cafe_settings" as never).select("allow_tips,allow_promos,allow_gift_cards,allow_scheduled_orders,tax_rate_bp,daily_message,daily_message_until").eq("store_id", s.id).maybeSingle(),
  ...
  const cartSubtotal = useMemo(
    () => cart.reduce((sum, line) => {
      ...
      return sum + (line.item.price_cents + modSum) * line.quantity;
    }, 0),
    [cart, modifiers],
  );
  ...
  const adjustQty = (uid: string, delta: number) =>
    setCart((p) => p.map((l) => l.uid === uid ? { ...l, quantity: Math.max(1, Math.min(99, l.quantity + delta)) } : l));
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="px-4 mt-4 mb-3 overflow-x-auto">
      ...
      <div className="px-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        {visibleItems.length === 0 ? (
          <p className="col-span-full text-sm text-muted-foreground text-center py-8">No items in this category yet.</p>
            ...
            <div className="flex">
              ...
              <div className="p-3 min-w-0 flex-1">
                ...
                <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-medium tabular-nums">{fmt(item.price_cents)}</span>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 inset-x-0 z-30 p-3 bg-gradient-to-t from-background via-background/95 to-background/0">
          <Button onClick={() => setCartOpen(true)} size="lg" className="w-full justify-between max-w-2xl mx-auto shadow-xl">
            <span className="flex items-center gap-2"><ShoppingBag className="h-4 w-4" /> {cart.reduce((s, l) => s + l.quantity, 0)} item{cart.length > 1 ? "s" : ""}</span>
                ...
                {(() => {
                  ...
                  return (
                    <Button onClick={addPickerToCart} disabled={invalid}>
                      Add — {fmt((pickerItem.price_cents + modSum) * pickerQty)}
                    </Button>
                  );
                })()}
              </DialogFooter>
            ...
            {cart.length === 0 ? (
              ...
              return (
                <div key={line.uid} className="flex gap-2 border-b border-border/40 pb-2 last:border-b-0">
                  ...
                  <span className="tabular-nums text-sm shrink-0">{fmt((line.item.price_cents + lineSum) * line.quantity)}</span>
                </div>
              );
            })}
            {cart.length > 0 && (
