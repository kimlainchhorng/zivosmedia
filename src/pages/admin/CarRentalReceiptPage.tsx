...
export default function CarRentalReceiptPage() {
  ...
  const [secureBookingUrl, setSecureBookingUrl] = useState<string | null>(null);
  const [secureLinkLoading, setSecureLinkLoading] = useState(true);
  const [secureLinkError, setSecureLinkError] = useState<string | null>(null);
  const [secureLinkAttempt, setSecureLinkAttempt] = useState(0);
  const { currency, format: formatMoney } = useStoreCurrency(
  ...
  const handlePrint = async () => {
    ...
    await printOrShareElement(
      mainRef.current,
      `receipt-${reservationId?.slice(0, 8) ?? "rental"}.pdf`,
      "Rental receipt",
    );
  };
      ...
      const [storeR, custR] = await Promise.all([
        ...
        r.customer_id
          ? supabase
              ...
              .select(
                "driver_license_number, driver_license_state, driver_license_country, driver_license_expiry, address, city, state",
              )
              .eq("id", r.customer_id)
              .maybeSingle()
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setSecureBookingUrl(null);
    setSecureLinkError(null);
    setSecureLinkLoading(true);
    (async () => {
      try {
        const access = await issueCarRentalReservationAccess(
          reservation.id,
          "manage",
        );
        ...
        if (!cancelled) {
          setSecureBookingUrl(`${window.location.origin}${path}`);
        }
      } catch (linkError) {
        if (!cancelled) {
          setSecureLinkError(
            linkError instanceof Error
              ? linkError.message
              : "A secure booking link could not be created.",
          );
        }
      } finally {
        if (!cancelled) setSecureLinkLoading(false);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [reservation?.id, secureLinkAttempt]);
  ...
  const balanceDue = Math.max(
    0,
    grandTotal - r.amount_paid_cents - r.deposit_paid_cents,
  );
      ...
      <header className="no-print sticky top-0 z-10 border-b border-border bg-card">
        <div className="mx-auto max-w-3xl px-4 py-3 flex items-center justify-between gap-3">
          <Link
            to={
              storeId
                ? `/admin/stores/${storeId}?tab=car-rental-reservations`
                : "/"
            }
          ...
          <div className="flex items-center gap-1.5">
            <Button
              ...
              onClick={async () => {
                ...
                doc.text(store?.name ?? "Car Rental Receipt", m, y);
                y += 22;
                doc.setFontSize(10);
                ...
                doc.text(r.vehicle_label, 300, y);
                y += 12;
                if (r.customer_phone) {
                  doc.text(r.customer_phone, m, y);
                  y += 12;
                }
                if (r.customer_email) {
                  doc.text(r.customer_email, m, y);
                  y += 12;
                }
                y += 8;
                ...
                doc.setFont("helvetica", "bold");
                doc.text("Pickup", m, y);
                doc.text("Drop-off", 300, y);
                y += 14;
                doc.setFont("helvetica", "normal");
                doc.text(new Date(r.pickup_at).toLocaleString(), m, y);
                doc.text(new Date(r.dropoff_at).toLocaleString(), 300, y);
                y += 12;
                if (r.pickup_location_name)
                  doc.text(r.pickup_location_name, m, y);
                if (r.dropoff_location_name)
                  doc.text(r.dropoff_location_name, 300, y);
                y += 18;
                doc.line(m, y, 576 - m, y);
                y += 14;
                ...
                const lines: Array<[string, string]> = [
                  [
                    `${r.rental_days} day(s) × ${formatMoney(r.daily_rate_cents)}`,
                    formatMoney(r.base_total_cents),
                  ],
                ];
                for (const a of addons)
                  lines.push([a.name, formatMoney(a.total_cents)]);
                if (r.fees_cents > 0)
                  lines.push(["Extra fees", formatMoney(r.fees_cents)]);
                if (r.discount_cents > 0)
                  lines.push(["Discount", `-${formatMoney(r.discount_cents)}`]);
                if (r.taxes_cents > 0)
                  lines.push(["Taxes", formatMoney(r.taxes_cents)]);
                if (r.security_deposit_cents > 0)
                  lines.push([
                ...
                doc.setFontSize(12);
                doc.text("Total", m, y);
                doc.text(formatMoney(grandTotal), 576 - m, y, {
                  align: "right",
                });
                ...
                doc.text(
                  `Generated by ZIVO Car Rental · ${new Date().toLocaleString()}`,
                  m,
                  760,
                );
                ...
                await exportBlob(
                  doc.output("blob"),
                  `receipt-${r.confirmation_code}.pdf`,
                  "Rental receipt",
                );
              }}
            >
      ...
      >
            ...
            {secureBookingUrl ? (
              <>
                ...
                <p className="mt-0.5 text-[9px] text-muted-foreground">
                  Secure link · scan to view booking
                </p>
              </>
            ) : secureLinkLoading ? (
              <div className="mt-2 flex items-center justify-end gap-1 text-[9px] text-muted-foreground">
                <Loader2 className="h-3 w-3 animate-spin" /> Preparing secure
                QR…
              </div>
            ) : (
              <div
                className="mt-2 max-w-[180px] text-[9px] text-destructive"
                role="alert"
              >
                <p>
                  {secureLinkError ?? "Secure booking QR unavailable."} No
                  unprotected QR was generated.
                </p>
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
                  className="no-print mt-1 h-6 px-2 text-[9px]"
                  onClick={() => setSecureLinkAttempt((attempt) => attempt + 1)}
                >
                  Try again
                </Button>
              </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 mb-5">
          <Block title="Renter">
            ...
            {customer?.driver_license_number && (
              <Row
                icon={IdCard}
                text={`License: ${customer.driver_license_number}${customer.driver_license_state ? ` (${customer.driver_license_state})` : ""}`}
              />
            )}
            {customer?.driver_license_expiry && (
              <p className="text-[11px] text-muted-foreground pl-6">
                Expires{" "}
                {new Date(customer.driver_license_expiry).toLocaleDateString()}
              </p>
            )}
          ...
          <Block title="Vehicle">
            ...
            {r.vehicle_category && (
              <p className="text-[11px] text-muted-foreground pl-6 capitalize">
                {r.vehicle_category}
              </p>
            )}
          </Block>
        ...
        <div className="grid gap-4 md:grid-cols-2 mb-5">
          <Block title="Pickup">
            ...
            {r.pickup_location_name && (
              <Row icon={MapPin} text={r.pickup_location_name} />
            )}
            {r.picked_up_at && (
              <p className="text-[11px] text-muted-foreground pl-6">
                Checked out: {formatDate(r.picked_up_at)}
                {r.pickup_odometer !== null &&
                  ` · ${r.pickup_odometer.toLocaleString()} mi`}
                {r.pickup_fuel_level !== null &&
                  ` · ${r.pickup_fuel_level}% fuel`}
              </p>
            )}
          </Block>
          <Block title="Drop-off">
            ...
            {r.dropoff_location_name && (
              <Row icon={MapPin} text={r.dropoff_location_name} />
            )}
            {r.returned_at && (
              <p className="text-[11px] text-muted-foreground pl-6">
                Returned: {formatDate(r.returned_at)}
                {r.dropoff_odometer !== null &&
                  ` · ${r.dropoff_odometer.toLocaleString()} mi`}
                {r.dropoff_fuel_level !== null &&
                  ` · ${r.dropoff_fuel_level}% fuel`}
              </p>
            )}
          </Block>
        </div>
        ...
        <div className="mb-5 rounded-xl border border-border bg-card print:bg-white">
          ...
          <div className="divide-y divide-border">
            <Line
              label={`Daily rate × ${r.rental_days} day${r.rental_days === 1 ? "" : "s"}`}
              value={formatMoney(r.base_total_cents)}
              sub={`${formatMoney(r.daily_rate_cents)}/day`}
            />
            {addons.map((a) => (
              <Line
                key={a.id}
                ...
                sub={`${a.quantity} × ${formatMoney(a.unit_price_cents)}${a.billing === "per_day" ? " /day" : ""}`}
              />
            ))}
            ...
            {r.fees_cents > 0 && (
              <Line
                label="Extra fees"
                value={formatMoney(r.fees_cents)}
                sub="Over-mileage, refuel, damage"
              />
            )}
            {r.discount_cents > 0 && (
              <Line
            ...
            {r.taxes_cents > 0 && (
              <Line label="Taxes" value={formatMoney(r.taxes_cents)} />
            )}
            ...
            <div className="border-t border-border px-4 py-2">
              {r.amount_paid_cents > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground">
                    {formatMoney(r.amount_paid_cents)}
                  </span>
                </div>
              )}
              {r.deposit_paid_cents > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground">
                    {formatMoney(r.deposit_paid_cents)}
                  </span>
                </div>
              )}
              {balanceDue > 0 && (
            ...
            <div className="border-t border-border bg-emerald-500/5 px-4 py-2">
              <div className="flex justify-between text-sm">
                <span className="font-semibold text-emerald-700 dark:text-emerald-300">
                  Refunded{" "}
                  {r.refund_at
                    ? `(${new Date(r.refund_at).toLocaleDateString()})`
                    : ""}
                </span>
        ...
        {(r.damage_marks?.length ?? 0) > 0 && (
          <div className="mb-5 rounded-xl border border-border bg-card p-3 print:bg-white print:break-inside-avoid">
            <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2">
              Damage diagram ({r.damage_marks.length} mark
              {r.damage_marks.length === 1 ? "" : "s"})
            </p>
            <div className="overflow-hidden rounded-lg border border-border bg-muted/20 print:bg-white">
              ...
              >
                ...
                {r.damage_marks.map((m, i) => (
                  <g
                    key={i}
                    transform={`translate(${(m.x / 100) * 200} ${(m.y / 100) * 100})`}
                  >
                    <circle
                      r="4"
                      className={cn(
        ...
        <section className="mt-6 rounded-xl border-2 border-foreground p-4 print:break-inside-avoid">
          ...
          <ul className="space-y-1 text-[11px] text-foreground/85 list-disc pl-4">
            <li>
              The Renter is responsible for the vehicle from pickup to return.
            </li>
            <li>
              Renter must hold a valid driver's license and proof of insurance.
            </li>
            <li>
              Vehicle must be returned with at least the same fuel level. A
              refuel charge applies otherwise.
            </li>
            <li>
              Renter accepts liability for damage caused during the rental
              period not covered by the elected insurance.
            </li>
            <li>Late returns may incur additional fees at the daily rate.</li>
            <li>Smoking is not permitted in any vehicle.</li>
            <li>
              Security deposit is refundable subject to the vehicle being
            ...
            <div>
              <div className="border-t border-foreground pt-1">
                <p className="text-[11px] font-bold">
                  {store?.name ?? "Authorized representative"}
                </p>
                <p className="text-[10px] text-muted-foreground">
        ...
        <p className="mt-6 text-center text-[10px] text-muted-foreground">
          Thank you for renting with {store?.name ?? "us"} · Receipt printed{" "}
  ...
  return (
    <div
      className={cn(
        ...
        bold && "bg-muted/20 font-semibold",
      )}
    >
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={cn(
        "mt-1 inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider",
        tone,
      )}
    >
      {status === "no_show"
