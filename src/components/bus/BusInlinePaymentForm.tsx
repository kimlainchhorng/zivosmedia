 ...
 * booking flow. Mirrors CarRentalInlinePaymentForm so the payment UX matches
 * other ZIVO surfaces. The bus PaymentIntent is authorize-only (manual
 * capture); the operator captures on confirmation via capture-bus-payment.
 */
...
function InnerPaymentForm({
  ...
  onCancel,
  onSuccess,
}: Omit<BusInlinePaymentFormProps, "clientSecret">) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="relative min-h-[180px] rounded-2xl border border-border/30 bg-card p-4">
        ...
        <div className={isElementReady ? "opacity-100 transition-opacity" : "opacity-0"}>
          <PaymentElement
            key={renderKey}
            onReady={() => { setIsElementReady(true); setLoadTimedOut(false); }}
            options={{
              layout: "tabs",
      ...
      <p className="px-2 text-center text-[11px] text-muted-foreground">
        We&apos;ll authorise the fare now — your card is only charged once the operator confirms your seats.
      </p>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button type="button" variant="outline" className="h-12 rounded-xl font-semibold" onClick={onCancel} disabled={isProcessing}>
          Back
        </Button>
        <Button type="submit" className="h-12 rounded-xl font-bold gap-2" disabled={!stripe || isProcessing || !isElementReady}>
          {isProcessing ? (
            <><Loader2 className="h-4 w-4 animate-spin" /> Processing…</>
          ) : (
            <><Lock className="h-4 w-4" /> Authorise {currencySymbol}{displayAmount}</>
          )}
        </Button>
      </div>
    </form>
...
export default function BusInlinePaymentForm({ clientSecret, amountCents, currency, onCancel, onSuccess }: BusInlinePaymentFormProps) {
  return (
    <Elements
      stripe={stripePromise}
      options={{ clientSecret, appearance: { theme: "stripe", variables: { borderRadius: "12px" } } }}
    >
      <InnerPaymentForm amountCents={amountCents} currency={currency} onCancel={onCancel} onSuccess={onSuccess} />
    </Elements>
  );
}
