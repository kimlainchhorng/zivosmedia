...
export default function StoreOwnerLayout({ children, title, storeId, storeName, storeLogoUrl, storeCategory, activeTab, onTabChange, productCount, orderCount, lodgingSetupProgress }: StoreOwnerLayoutProps) {
  ...
  const isBus = normalizedStoreCategory.includes("bus");
  ...
  const navItems = [
    ...
    // Bus operators manage routes/trips/bookings in the dedicated console.
    ...(isBus ? [{ id: "_bus_console", label: "Bus Routes & Trips", icon: Bus, external: "/bus/operator" }] : []),
          ...
          <div className="space-y-0.5" role="group" aria-labelledby="sidebar-group-manage">
            {navItems.map((item: any) => {
              ...
              return (
                <button type="button"
                  key={item.id}
                  onClick={() => { if (item.external) { navigate(item.external); } else { onTabChange?.(item.id); } closeSidebar(); }}
                  className={cn(
