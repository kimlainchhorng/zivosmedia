  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<PayoutRow[]> => {
      const { data, error } = await (
        supabase.from("lodge_payout_requests") as any
      )
        .select(
          "id,amount_cents,currency,rail,status,reference,failure_reason,note,created_at,paid_at",
        )
        .eq("store_id", storeId)
    ...
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">
          No payout requests yet. Click "Request payout" to send one.
        </p>
      ) : (
                    ...
                    <td className="py-2 text-[11px] text-muted-foreground max-w-[220px]">
                      {r.status === "rejected" ? (
                        <span className="text-red-700">
                          {r.failure_reason || "Payout outcome unavailable"}
                        </span>
                      ) : r.reference ? (
