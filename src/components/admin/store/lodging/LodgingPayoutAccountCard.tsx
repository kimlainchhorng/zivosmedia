...
export default function LodgingPayoutAccountCard({
  storeId,
  storeCountry,
}: Props) {
  const { user } = useAuth();
  ...
  const { data: methods = [], isLoading: methodsLoading } = useQuery({
    ...
    queryFn: async (): Promise<PayoutMethodRow[]> => {
      return (await loadOwnPayoutMethods(
        "store",
        storeId,
      )) as PayoutMethodRow[];
    },
  ...
  const startStripe = () => {
    if (!rails.stripe) {
      ...
      toast.error(
        `Stripe Connect isn't available in ${country}. Switched to ${fallback.replace("_", " ")}.`,
      );
      return;
    }
    onboard.mutate(country, {
      onError: (e: any) => {
        ...
        if (
          msg.includes("stripe_unsupported_country") ||
          msg.toLowerCase().includes("not available")
        ) {
          const fallback = recommended === "stripe" ? "bank_wire" : recommended;
          setActiveRail(fallback);
          toast.error(
            `Stripe Connect rejected ${country}. Switched to ${fallback.replace("_", " ")}.`,
          );
        }
      },
    });
  ...
  const removeMethod = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["lodge-payout-methods"] });
    },
    ...
    <>
      ...
      <div className="rounded-lg border border-border bg-card p-4 space-y-3">
        ...
        <p className="text-[11px] text-muted-foreground">
          Property country: <span className="font-medium">{country}</span> ·
          Recommended rail:{" "}
          <span className="font-medium">{recommended.replace("_", " ")}</span>
        </p>

        <Tabs
          value={activeRail}
          onValueChange={(v) => setActiveRail(v as PayoutRail)}
        >
          <TabsList
            className="grid w-full"
            style={{
              gridTemplateColumns: `repeat(${[rails.stripe, rails.aba, rails.bank_wire, rails.paypal].filter(Boolean).length}, minmax(0, 1fr))`,
            }}
          >
            {rails.stripe && (
            ...
            {rails.aba && (
              <TabsTrigger value="aba" className="text-xs">
                ABA
              </TabsTrigger>
            )}
            {rails.bank_wire && (
              <TabsTrigger value="bank_wire" className="text-xs">
                Bank wire
              </TabsTrigger>
            )}
            {rails.paypal && (
              ...
              {connectLoading ? (
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap gap-1.5">
                    <Badge
                      variant={
                        connect.details_submitted ? "default" : "secondary"
                      }
                      className="gap-1"
                    >
                      ...
                      Details{" "}
                      {connect.details_submitted ? "submitted" : "pending"}
                    </Badge>
                    <Badge
                      variant={
                        connect.payouts_enabled ? "default" : "secondary"
                      }
                      className="gap-1"
                    >
...
function ManualMethodForm({
  ...
  const ownMethods = methods.filter(
    (m) => (m.rail || mapMethodTypeToRail(m.method_type)) === rail,
  );
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (!confirmed)
        throw new Error("Please confirm the account details are correct");

      // Per-rail validation (eligibility + format)
      if (rail === "aba") {
        if (!form.aba_account_id.trim())
          throw new Error("ABA account ID is required");
        if (!form.account_holder_name.trim())
          throw new Error("Account holder name is required");
        if (!/^[0-9]{6,12}$/.test(form.aba_account_id.replace(/\s/g, "")))
          throw new Error("ABA account ID should be 6–12 digits");
      }
      if (rail === "bank_wire") {
        ...
        if (!form.account_holder_name.trim())
          throw new Error("Account holder name is required");
        if (country !== "US" && !form.swift.trim())
          throw new Error("SWIFT/BIC is required for international wires");
      }
      if (rail === "paypal" && !/.+@.+\..+/.test(form.paypal_email))
        throw new Error("Valid PayPal email required");

      const method_type =
        rail === "aba" ? "aba" : rail === "paypal" ? "paypal" : "bank_transfer";
      const payload: Record<string, any> = {
        ...
        verification_note:
          rail === "bank_wire" && form.swift
            ? `SWIFT/BIC: ${form.swift}`
            : null,
        ...
        {ownMethods.length > 0 && (
          <ul className="space-y-1.5">
            {ownMethods.map((m) => (
              <li
                key={m.id}
                className="flex items-center justify-between rounded-md border border-border bg-background/50 px-2.5 py-1.5"
              >
                <div className="min-w-0">
                  <p className="text-xs font-semibold truncate flex items-center gap-1.5">
                    {m.label || defaultLabel(rail)}
                    {m.verification_status === "verified" ? (
                      <Badge className="bg-emerald-500/15 text-emerald-700 border-emerald-500/30 h-4 px-1 text-[9px]">
                        Verified
                      </Badge>
                    ) : (
                  ...
                  <p className="text-[11px] text-muted-foreground truncate">
                    {m.method_type === "aba"
                      ? `ABA •••• ${m.destination_last4 || "unavailable"}`
                      : m.method_type === "paypal"
                        ? `PayPal •••• ${m.destination_last4 || "unavailable"}`
                        : `${m.bank_name || "Bank"} •••• ${m.destination_last4 || "unavailable"}`}
                  </p>
                </div>
                <Button
                  size="icon"
          ...
          <div className="space-y-2 rounded-md border border-border bg-background/40 p-2.5">
            ...
            {rail === "aba" && (
              <FieldRow label="ABA account ID / phone">
                <Input
                  value={form.aba_account_id}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, aba_account_id: e.target.value }))
                  }
            ...
            <label className="flex items-start gap-2 text-[11px] text-muted-foreground cursor-pointer">
              ...
              />
              <span>
                I confirm these account details are correct and the holder name
                matches my ID. ZIVO will mark this method as verified after the
