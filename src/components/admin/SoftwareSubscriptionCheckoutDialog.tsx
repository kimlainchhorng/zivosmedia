...
import { AlertCircle, Check, ExternalLink, Loader2, Lock, Shield } from "lucide-react";
...
export default function SoftwareSubscriptionCheckoutDialog({
  ...
  const selectedPlanId = cycle === "annual" ? plan.annualPlanId : plan.monthlyPlanId;
      ...
      <DialogContent className="max-h-[92vh] max-w-md overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Lock className="h-4 w-4 text-emerald-600" /> ZIVO {plan.displayName} checkout
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-2 rounded-xl border bg-muted/30 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold">ZIVO {plan.displayName}</p>
          ...
          <ul className="space-y-1.5 border-t pt-3 text-xs text-muted-foreground">
            {plan.trialDays > 0 && (
              <li className="flex items-start gap-2">
                ...
                {formatTrialLabel(plan.trialDays)}; Stripe confirms the first charge date before you submit.
              </li>
            )}
            <li className="flex items-start gap-2">
              <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-600" />
        ...
        <p className="text-xs text-muted-foreground">
          Your selected plan is revalidated against the active server catalog. You will review the
          final price, trial, renewal date, and payment method on Stripe before subscribing.
        </p>
        ...
        <Button className="min-h-11 w-full gap-2" onClick={startCheckout} disabled={redirecting}>
          {redirecting ? (
            <><Loader2 className="h-4 w-4 animate-spin" /> Opening secure checkout…</>
          ) : (
            <><Lock className="h-4 w-4" /> Continue to Stripe <ExternalLink className="h-3.5 w-3.5" /></>
          )}
        </Button>
        <Button variant="outline" className="min-h-11 w-full" onClick={() => onOpenChange(false)} disabled={redirecting}>
          Cancel
        </Button>
        ...
        <p className="text-center text-[11px] text-muted-foreground">
          ...
          <a href={`${ZIVO_SOFTWARE_ORIGIN}/terms-of-service`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Terms</a>
          {" "}and{" "}
          <a href={`${ZIVO_SOFTWARE_ORIGIN}/privacy-policy`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Privacy Policy</a>.
        </p>
      </DialogContent>
    </Dialog>
  );
