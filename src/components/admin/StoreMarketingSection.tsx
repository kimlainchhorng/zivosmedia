...
export default function StoreMarketingSection({ storeId, storeSlug, storeName, storeCategory }: Props) {
  ...
  const savePromo = useMutation({
    mutationFn: async (isEdit: boolean) => {
      // Promotion writes are server-gated (20260601151500_promotions_server_gate
      // revokes direct authenticated INSERT/UPDATE/DELETE); route through the
      // promotion-manage edge function so owners/admins are verified server-side.
      const promotion = {
        name: promoForm.name,
      ...
      const { data, error } = isEdit && editingPromo
        ? await supabase.functions.invoke("promotion-manage", {
            body: { action: "update", promotion_id: editingPromo.id, promotion },
          })
        : await supabase.functions.invoke("promotion-manage", {
            body: { action: "create", merchant_id: storeId, promotion },
          });
      if (error) throw error;
