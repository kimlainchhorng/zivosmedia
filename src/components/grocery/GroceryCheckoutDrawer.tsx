...
import {
  ...
  getPriorityFee,
  type SchedulerState,
} from "@/components/grocery/GroceryDeliveryScheduler";
...
const CHECKOUT_PROFILE_STORAGE_KEY = "zivo_checkout_profile";
const GUEST_CHECKOUT_SCOPE = "guest";

function checkoutProfileStorageKey(userId: string | null): string {
  return `${CHECKOUT_PROFILE_STORAGE_KEY}:${userId ?? GUEST_CHECKOUT_SCOPE}`;
}
...
function getSavedProfile(userId: string | null): {
  name: string;
  phone: string;
  subPref: SubstitutionPref;
} {
  try {
    const raw = localStorage.getItem(checkoutProfileStorageKey(userId));
    if (raw) {
      ...
      return {
        ...
        subPref:
          parsed.subPref === "best_match" || parsed.subPref === "refund"
            ? parsed.subPref
            : "contact_me",
      };
    }
...
export function GroceryCheckoutDrawer({
  ...
  storePaymentTypes = ["cash", "card"],
}: GroceryCheckoutDrawerProps) {
  const { t } = useI18n();
  const { user } = useAuth();
  const { data: userProfile } = useUserProfile();
  const { getCurrentLocation, reverseGeocode, isGettingLocation } =
    useCurrentLocation();
  const userId = user?.id ?? null;
  const {
    addresses: savedAddresses,
    selectedAddress,
    isLoaded: savedAddressesLoaded,
    addAddress,
    removeAddress,
    selectAddress,
  } = useDeliveryAddress();
  const savedProfile = getSavedProfile(userId);
  ...
  const [addressSuggestions, setAddressSuggestions] = useState<
    { display: string; mainText: string; placeId: string }[]
  >([]);
  ...
  useEffect(() => {
    if (!savedAddressesLoaded || !selectedAddress || address) return;
    setAddress(selectedAddress.address);
    void forwardGeocode(selectedAddress.address).then((coords) => {
      if (coords) setAddressCoords(coords);
    });
  }, [address, savedAddressesLoaded, selectedAddress]);
  ...
  const [paymentTotalCents, setPaymentTotalCents] = useState<number | null>(
    null,
  );
  ...
  const [subPref, setSubPref] = useState<SubstitutionPref>(
    savedProfile.subPref,
  );
  ...
  const [routeMetrics, setRouteMetrics] = useState<GroceryRouteMetrics | null>(
    null,
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setRouteMetrics({
        ...
        durationMinutes:
          route.duration_in_traffic_minutes ?? route.duration_minutes,
      });
    })();
  ...
  const pickupCity = useMemo(() => {
    ...
    const parts = address
      ...
      .filter(Boolean);
    if (parts.length >= 3)
      return parts[parts.length - 3] || parts[parts.length - 2] || "default";
  ...
  const effectiveDistanceMiles =
    routeMetrics?.distanceMiles ?? (isCambodia ? 5 * 0.621371 : 3);
  ...
  const deliveryFee = useMemo(() => {
    ...
    if (isCambodia && motoPricing) {
      ...
      const raw =
        motoPricing.base_fare +
        motoPricing.per_mile * effectiveDistanceMiles +
        motoPricing.booking_fee;
    ...
    if (motoPricing) {
      const raw =
        motoPricing.base_fare +
        motoPricing.per_mile * effectiveDistanceMiles +
        motoPricing.per_minute * effectiveDurationMinutes +
        motoPricing.booking_fee;
      return Math.round(Math.max(motoPricing.minimum_fare, raw) * 100) / 100;
    }
  ...
  const selectAddressSuggestion = useCallback(
    async (s: { display: string; placeId?: string }) => {
  ...
  useEffect(() => {
    if (name || phone) {
      localStorage.setItem(
        checkoutProfileStorageKey(userId),
        JSON.stringify({ name, phone, subPref }),
      );
    }
  }, [name, phone, subPref, userId]);
  ...
  const handleCreateInlinePayment = async () => {
    ...
    try {
      ...
      if (error)
        throw new Error(
          error.message || "Failed to initialize secure card checkout",
        );
      ...
      setPaymentTotalCents(
        Number(data.total_cents || Math.round(grandTotal * 100)),
      );
    } catch (err: any) {
  ...
  const handleInlinePaymentSuccess = useCallback(
    async (paymentIntentId: string) => {
      ...
      try {
        ...
        toast.success(
          earnedPoints > 0
            ? `Payment successful • +${earnedPoints} loyalty points`
            : "Payment successful",
        );
        ...
        toast.error(
          err.message ||
            "Payment completed but confirmation failed. Please contact support.",
        );
      } finally {
        setIsSubmitting(false);
      }
  ...
  const handlePaypalOrSquareOrder = async (provider: "paypal" | "square") => {
    ...
    try {
      ...
      if (error)
        throw new Error(
          error.message || `${provider} checkout could not start`,
        );
      if ((data as any)?.error) throw new Error((data as any).error);
      const redirect =
        provider === "paypal" ? (data as any).approve_url : (data as any).url;
      if (!redirect)
  ...
  const SUB_OPTIONS: {
  ...
  }[] = [
    {
      value: "contact_me",
      label: "Contact Me",
      desc: "Driver will message you",
      icon: MessageSquare,
    },
    {
      value: "best_match",
      label: "Best Match",
      desc: "Pick a similar item",
      icon: RefreshCw,
    },
    {
      value: "refund",
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 min-h-0 px-5 pb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold tracking-tight">
                  {step === 1
                    ? t("grocery.checkout.delivery_details")
                    : t("grocery.checkout.review_pay")}
                </h2>
                <p className="text-[11px] text-muted-foreground">
                  {t("grocery.checkout.step_of")} {step} / 2 · {itemCount}{" "}
                  {t("grocery.checkout.items_from")} {storeName}
                </p>
              </div>
            </div>
            <motion.button
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div
              className={`flex-1 h-1.5 rounded-full transition-all duration-500 ${step >= 2 ? "bg-primary" : "bg-muted/40"}`}
            />
          </div>
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-primary/5 border border-primary/10 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {isCambodia
                        ? t("grocery.checkout.driver_will_shop_kh") ||
                          "Driver will shop & deliver by motorcycle/tuk tuk"
                ...
                {profileAutoFilled && userProfile?.full_name && (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400">
                      {t("grocery.checkout.contact_autofilled") ||
                        "Contact auto-filled from your account"}
                    </span>
                  </motion.div>
                )}
                ...
                {address.trim().length > 5 &&
                  !savedAddresses.some((sa) => sa.address === address) && (
                    <AnimatePresence>
                      {!showSaveForm ? (
                        <motion.button
                          initial={{ opacity: 0 }}
                ...
                >
                  ...
                  <span className="text-[12px] font-medium">
                    {t("grocery.checkout.leave_at_door")}
                  </span>
                </motion.button>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {SUB_OPTIONS.map((opt) => (
                    ...
                    >
                      <opt.icon
                        className={`h-4 w-4 mx-auto mb-1 ${subPref === opt.value ? "text-primary" : "text-muted-foreground/50"}`}
                      />
                      <p
                        className={`text-[10px] font-bold ${subPref === opt.value ? "text-primary" : "text-foreground/70"}`}
                      >
                        {opt.label}
                      </p>
                            ...
                            >
                              ...
                              {onUpdateQuantity ? (
                                <div className="flex items-center gap-1.5 shrink-0">
                                  ...
                                  <span className="font-semibold tabular-nums ml-1 w-14 text-right">
                ...
                <div className="rounded-2xl bg-muted/15 border border-border/20 p-3.5 mb-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {!isCambodia && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-muted-foreground">
                          Platform fee ({markupPct}%)
                        </span>
                        <span className="text-foreground font-medium tabular-nums">
                          ${platformMarkup.toFixed(2)}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between text-[12px]">
                      <span className="text-muted-foreground flex items-center gap-1.5">
                        ...
                        {t("grocery.checkout.delivery_fee") || "Delivery Fee"} (
                        {distanceLabel})
                      </span>
                      <span className="text-foreground font-medium tabular-nums">
                    ...
                    {!isCambodia && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-muted-foreground flex items-center gap-1.5">
                          {t("grocery.checkout.service_fee") || "Service Fee"} (
                          {SERVICE_FEE_PCT}%)
                          {isPlus && (
                            <span className="text-[8px] font-bold px-1 py-0.5 rounded bg-amber-500/15 text-amber-600">
                              ZIVO+
                            </span>
                          )}
                        </span>
                        {isPlus ? (
                          <span className="text-amber-600 font-bold">
                            <span className="line-through text-muted-foreground/50 mr-1">
                              ${calcServiceFee(total).toFixed(2)}
                            </span>
                            $0.00
                          </span>
                        ) : (
                    ...
                    {priorityFee > 0 && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-muted-foreground flex items-center gap-1.5">
                          ⚡ {t("grocery.checkout.priority") || "Priority"}
                        </span>
                    ...
                    <div className="border-t border-border/15 pt-1.5 mt-1.5 flex justify-between text-[13px] font-bold">
                      <span>
                        {t("grocery.checkout.estimated_total") ||
                          "Estimated Total"}
                      </span>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-muted-foreground font-medium">
                        Substitutions:{" "}
                        {SUB_OPTIONS.find((o) => o.value === subPref)?.label}
                      </span>
                    </div>
                  </div>
                  {deliveryNote && (
                  ...
                  <div className="px-3.5 pb-3.5 pt-2 border-t border-border/10 space-y-1.5">
                    ...
                    <div className="flex justify-between text-[12px] text-muted-foreground">
                      <span className="flex items-center gap-1.5">
                        <Truck className="h-3 w-3" />{" "}
                        {isCambodia ? "Delivery" : "Delivery"} ({distanceLabel})
                      </span>
                      <span className="text-foreground tabular-nums">
                        ${deliveryFee.toFixed(2)}
                      </span>
                    ...
                    {tip > 0 && (
                      <div className="flex justify-between text-[12px] text-muted-foreground">
                        <span className="flex items-center gap-1.5">
                          <Heart className="h-3 w-3 text-foreground" /> Driver
                          tip
                        </span>
                        <span className="text-foreground tabular-nums">
                          ${tip.toFixed(2)}
                    ...
                    {promoDiscount > 0 && (
                      <div className="flex justify-between text-[12px] text-emerald-600">
                        <span className="flex items-center gap-1.5">
                          <Gift className="h-3 w-3" /> Promo discount
                        </span>
                  ...
                  <GroceryPromoInput
                    onApply={(code, discount) => {
                      ...
                      if (paymentClientSecret) {
                        resetInlinePayment();
                        toast.success(
                          "Promo applied. Total updated — tap Pay again.",
                        );
                      }
                    }}
                  />
                ...
                {paymentClientSecret ? (
                  <div className="rounded-2xl bg-muted/10 border border-border/20 p-3.5 mb-3">
                    ...
                    <GroceryInlinePaymentForm
                      ...
                      totalCents={
                        paymentTotalCents ?? Math.round(grandTotal * 100)
                      }
                    ...
                    <div className="space-y-1.5 mb-3 px-2">
                      <div className="flex items-center gap-2 p-2.5 rounded-xl bg-muted/20 border border-border/10">
                        ...
                        <p className="text-[9px] text-muted-foreground leading-relaxed">
                          ...
                          Report quality issues within 24h for a full refund.{" "}
                          <Link
                            to="/grocery/returns"
                      ...
                      <div className="flex items-center gap-2 p-2.5 rounded-xl bg-muted/20 border border-border/10">
                        ...
                        <p className="text-[9px] text-muted-foreground leading-relaxed">
                          ...
                          Free before driver assigned · 15–50% after.{" "}
                          <Link
                            to="/grocery/fees"
                            className="text-primary/70 underline"
                          >
        ...
        {/* Sticky CTA */}
        <div
          className="shrink-0 px-5 pt-3 pb-20 border-t border-border/10 bg-background/95 backdrop-blur-md"
          style={{
            paddingBottom:
              "max(5rem, calc(var(--zivo-safe-bottom,0px) + 5rem))",
          }}
        >
          {step === 1 ? (
            ...
            <motion.div whileTap={!isSubmitting ? { scale: 0.97 } : {}}>
              <Button
                ...
                onClick={
                  selectedPayment === "card"
                    ? handleCreateInlinePayment
                    : selectedPayment === "paypal"
                      ? () => handlePaypalOrSquareOrder("paypal")
                      : selectedPayment === "square"
                        ? () => handlePaypalOrSquareOrder("square")
            ...
            <span className="text-[9px] text-muted-foreground/40">
              {selectedPayment === "cash"
                ...
                : selectedPayment === "aba"
                  ? "ABA Bank · Secure QR"
                  : selectedPayment === "paypal"
                    ? "PayPal · Secure redirect"
                    : selectedPayment === "square"
                      ? "Square · Secure redirect"
