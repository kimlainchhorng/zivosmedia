import { installRealtimeCircuit, realtimeBackoff } from "@/lib/realtime/connectionCircuit";
...
const sharedClientOptions = {
  ...
  realtime: {
    ...
    reconnectAfterMs: realtimeBackoff,
    timeout: 30_000,
  },
} as const;

/**
 * Realtime may use a different key from REST, without touching REST.
 *
 * Set VITE_SUPABASE_REALTIME_KEY (for example to the legacy anon JWT) and
 * rebuild — Vite bakes env at build time. Left unset, Realtime keeps using the
 * client's own key.
 *
 * Before reaching for this: the publishable key is NOT rejected by this
 * project's Realtime tenant. A real WebSocket handshake with it reaches OPEN
 * and joins a channel with status "ok", from Node and from a browser under the
 * production CSP, on both vsn=1.0.0 and vsn=2.0.0. `error code: 1101` from a
 * plain GET is what a *valid* key returns — the request passes the apikey gate
 * and then hits the socket handler with no Upgrade header. The keys that answer
 * a clean 401 are the rejected ones. See
 * docs/realtime-key-diagnosis-2026-09-09.md, and
 * `npm run qa:realtime-handshake` to re-check in one command.
 */
const REALTIME_KEY_OVERRIDE = (import.meta.env.VITE_SUPABASE_REALTIME_KEY || "").trim();

function realtimeOptionsFor(clientKey: string) {
  return REALTIME_KEY_OVERRIDE
    ? { ...sharedClientOptions.realtime, params: { apikey: REALTIME_KEY_OVERRIDE } }
    : { ...sharedClientOptions.realtime, params: { apikey: clientKey } };
}

export const authSupabase = createClient<Database>(
  EFFECTIVE_AUTH_SUPABASE_URL,
  EFFECTIVE_AUTH_SUPABASE_KEY,
  { ...sharedClientOptions, realtime: realtimeOptionsFor(EFFECTIVE_AUTH_SUPABASE_KEY) },
);

export const dataSupabase = createClient<Database>(EFFECTIVE_DATA_SUPABASE_URL, EFFECTIVE_DATA_SUPABASE_KEY, {
  ...sharedClientOptions,
  realtime: realtimeOptionsFor(EFFECTIVE_DATA_SUPABASE_KEY),
  auth: {
...
installRealtimeCircuit(authSupabase.realtime);
installRealtimeCircuit(dataSupabase.realtime);
