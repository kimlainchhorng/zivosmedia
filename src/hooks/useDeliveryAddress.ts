...
function loadLocalAddresses(userId: string | null): DeliveryAddress[] {
  ...
  try {
    const raw = window.localStorage.getItem(
      scopedStorageKey(STORAGE_KEY, userId),
    );
    ...
    return parsed.filter((value): value is DeliveryAddress => {
      ...
      return (
        ...
        (candidate.label === "Home" ||
          candidate.label === "Work" ||
          candidate.label === "Other") &&
...
export function useDeliveryAddress() {
  ...
  const addresses =
    addressesState.userId === userId
      ? addressesState.items
      : loadLocalAddresses(userId);
  const selectedId =
    selectedState.userId === userId
      ? selectedState.value
      : loadSelectedId(userId);

  const updateAddresses = useCallback(
    (
      updater:
        | DeliveryAddress[]
        | ((previous: DeliveryAddress[]) => DeliveryAddress[]),
    ) => {
  ...
  const replaceAddresses = useCallback(
  ...
  const setActiveSelectedId = useCallback(
    (next: string | null | ((previous: string | null) => string | null)) => {
      setSelectedState((previous) => {
        const current =
          previous.userId === userId ? previous.value : loadSelectedId(userId);
    ...
    const loadFromSupabase = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setActiveSelectedId(
            cachedSelectedId &&
              mapped.some((address) => address.id === cachedSelectedId)
              ? cachedSelectedId
        ...
        if (active)
          console.error(
            "[useDeliveryAddress] Failed to load from Supabase:",
            err,
          );
      } finally {
  ...
  const addAddress = useCallback(
    async (addr: Omit<DeliveryAddress, "id">) => {
      ...
      if (userId) {
        try {
          ...
          console.error(
            "[useDeliveryAddress] Failed to save to Supabase:",
            err,
          );
        }
      }
  ...
  const updateAddress = useCallback(
    async (id: string, updates: Partial<DeliveryAddress>) => {
      updateAddresses((prev) =>
        prev.map((a) => {
      ...
      if (userId) {
        try {
          ...
          console.error(
            "[useDeliveryAddress] Failed to update in Supabase:",
            err,
          );
        }
      }
    ...
    async (id: string) => {
      ...
      if (userId) {
        try {
          ...
          console.error(
            "[useDeliveryAddress] Failed to delete from Supabase:",
            err,
          );
        }
      }
