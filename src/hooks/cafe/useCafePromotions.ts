...
export function useCafePromotions(storeId: string | undefined): UseCafePromotionsResult {
  ...
  const create = useCallback(async (draft: CafePromotionDraft) => {
    ...
    const { data, error: err } = await supabase.functions.invoke("cafe-promotion-manage", {
    ...
    const created = data?.promotion as CafePromotion;
  ...
  const update = useCallback(async (id: string, patch: Partial<CafePromotionDraft & { sort_order: number }>) => {
    ...
    const { error: err } = await supabase.functions.invoke("cafe-promotion-manage", {
      body: { action: "update", promotion_id: id, promotion: patch },
    });
    setSaving(false);
