 ...
 *
 * Wraps server-gated writes for `chat_group_members`, `chat_groups`,
...
export function useGroupAdmin(groupId: string | null) {
  ...
  const setRole = useCallback(async (targetUserId: string, role: GroupRole) => {
    ...
    const { error } = await setGroupMemberRole({ group_id: groupId, user_id: targetUserId, role });
  ...
  const kick = useCallback(async (targetUserId: string) => {
    if (!groupId) return false;
    const { error } = await removeGroupMember({ group_id: groupId, user_id: targetUserId });
  ...
  const muteUntil = useCallback(async (targetUserId: string, until: Date | null) => {
    if (!groupId) return false;
    const { error } = await muteGroupMember({
      group_id: groupId,
      user_id: targetUserId,
      muted_until: until ? until.toISOString() : null,
    });
  ...
  const updateGroupMeta = useCallback(async (patch: { name?: string; avatar_url?: string | null }) => {
    if (!groupId) return false;
    const { error } = await updateGroup({ group_id: groupId, ...patch });
  ...
  const addMembers = useCallback(async (memberIds: string[]) => {
    if (!groupId || memberIds.length === 0) return false;
    const { error } = await addGroupMembers({ group_id: groupId, member_ids: memberIds });
    if (error) { toast.error(error.message || "Could not add members"); return false; }
    toast.success(memberIds.length === 1 ? "Member added" : "Members added");
    await refresh();
    return true;
  }, [groupId, refresh]);
  ...
  const createInvite = useCallback(async (opts?: { expiresInHours?: number; maxUses?: number }) => {
    ...
    await refresh();
    return (data?.invite ?? null) as GroupInviteRow | null;
  }, [groupId, user?.id, refresh]);

  const revokeInvite = useCallback(async (inviteId: string) => {
    const { error } = await revokeGroupInvite({ invite_id: inviteId });
  ...
  return {
    ...
    refresh, setRole, kick, muteUntil, leave, addMembers,
