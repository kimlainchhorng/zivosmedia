...
describe("payment webhook and payout idempotency contracts", () => {
  it("keeps Stripe webhook purchase writes idempotent by transaction id", () => {
    ...
    expect(text).toMatch(/withSecurity\(\s*["']stripe-webhook["']/);
  ...
  it("fails car rental charging closed while keeping refunds idempotent", () => {
    ...
    expect(deposit).toContain("status: 503");
    expect(deposit).not.toContain("Stripe");
    expect(deposit).not.toContain("paymentIntents");
    expect(deposit).not.toContain("req.json");
