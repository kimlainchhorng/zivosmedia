...
describe("security, anti-abuse, and hacker-protection workflow", () => {
  it("keeps public app auth redirects on the modern login/signup flow", () => {
    ...
    expect(read("src/pages/store/ServiceBookingPage.tsx")).toContain(
      'withRedirectParam("/signup", `/store/${slug}`)',
    );
    expect(read("src/pages/SocialFeedPage.tsx")).toContain(
      'withRedirectParam("/login", "/feed")',
    );
  });
  ...
  it("keeps OTP and provider webhook endpoints on strict wrappers without breaking provider auth", () => {
    ...
    const stripeCarRentalWebhook = read(
      "supabase/functions/stripe-car-rental-webhook/index.ts",
    );
  ...
  it("keeps bot and channel communication endpoints on strict wrappers without breaking webhook callers", () => {
    ...
    const channelBroadcast = read(
      "supabase/functions/channel-broadcast/index.ts",
    );
    ...
    expect(channelBroadcast).toContain(
      'sub?.role === "admin" || sub?.role === "editor"',
    );
    expect(channelBroadcast).toContain("comments_enabled");
    expect(channelBroadcast).toContain(
      "comments_enabled === false ? false : true",
    );
    expect(channelBroadcast).toContain("normalizedText");
    expect(channelBroadcast).toContain("normalizedMedia.length === 0");
    expect(channelBroadcast).toContain(
      "scheduled_for must be a future ISO timestamp",
    );

    const scheduled = read(
      "supabase/functions/channel-publish-scheduled/index.ts",
    );
  ...
  it("keeps AR communications and Facebook marketing actions behind strict wrappers", () => {
    ...
    const cron = read(
      "supabase/migrations/20260609000000_ar_reminders_secure_cron.sql",
    );
  ...
  it("blocks scanners, scrapers, injection payloads, risky networks, and repeat threat actors", () => {
    ...
    expect(threatMigration).toContain(
    ...
    expect(autoBlockMigration).toContain(
      "CREATE OR REPLACE FUNCTION public.auto_block_if_high_threat",
    );
    expect(autoBlockMigration).toContain(
      "GRANT EXECUTE ON FUNCTION public.auto_block_if_high_threat",
    );
    expect(autoBlockMigration).toContain("TO service_role");

    expect(networkMigration).toContain(
  ...
  it("covers account takeover, card testing, spam, scraping, fake booking, and key-leak drills", () => {
    ...
    const marketingDispatch = read(
      "supabase/functions/notify-dispatch/index.ts",
    );
    const publicSalonBookingPage = read(
      "src/pages/salon/PublicSalonBookingPage.tsx",
    );
    ...
    expect(publicBookingSecurity).toContain(
    ...
    expect(publicSalonBookingPage).not.toMatch(
    ...
    expect(publicBookingGate).toContain(
  ...
  it("keeps security events, incident escalation, and admin-only acknowledgement auditable", () => {
    ...
    const securityReport = read(
      "supabase/functions/security-report-submit/index.ts",
    );
    const securityReportGate = read(
      "supabase/migrations/20260601023000_security_reports_server_gate.sql",
    );
    ...
    expect(securityReportGate).toContain(
      "COALESCE(category, 'general') <> 'security_report'",
    );
    expect(securityReportGate).toContain(
      "trusted server-side ingestion through security-report-submit",
    );

    expect(securityReportPage).toContain(
      'functions.invoke("security-report-submit"',
    );
    expect(securityReportPage).not.toMatch(
      /from\("feedback_submissions"\)\.insert/,
    );
  });
  ...
  it("keeps social safety reports behind trusted server-side intake", () => {
    ...
    const safetyGate = read(
      "supabase/migrations/20260601054500_social_safety_reports_server_gate.sql",
    );
    const storyChatGate = read(
      "supabase/migrations/20260601060000_story_chat_safety_reports_server_gate.sql",
    );
    ...
    for (const surface of [
      ...
      expect(surface).not.toMatch(
        /from\("(post_reports|comment_reports|content_reports|group_message_reports|chat_message_reports|story_reports|story_comment_reports)"\)[\s\S]{0,140}\.insert/,
      );
    }
  });
  ...
  it("keeps moderation appeals behind verified server-side submission", () => {
    const appealFn = read(
      "supabase/functions/moderation-appeal-submit/index.ts",
    );
    const appealGate = read(
      "supabase/migrations/20260601061500_moderation_appeals_server_gate.sql",
    );
    ...
    expect(appealsPage).toContain(
      'functions.invoke("moderation-appeal-submit"',
    );
    expect(appealsPage).not.toMatch(
      /from\("appeal_requests"\)[\s\S]{0,160}\.insert/,
    );
  });
  ...
  it("keeps admin moderation reviews behind server-side admin audit", () => {
    ...
    expect(moderationPage).not.toMatch(
      /from\("content_moderation_queue"\)[\s\S]{0,220}\.update/,
    );
    expect(moderationPage).not.toMatch(
      /from\("moderation_actions"\)[\s\S]{0,220}\.insert/,
    );
  });
  ...
  it("keeps admin content report status changes behind server-side admin review", () => {
    ...
    expect(reportsPage).not.toMatch(
      /from\("content_reports"\)[\s\S]{0,220}\.update/,
    );
  });

  it("keeps local account security setting writes behind server-side user scoping", () => {
    const settingsFn = read(
      "supabase/functions/account-security-settings/index.ts",
    );
    const settingsGate = read(
      "supabase/migrations/20260601070000_account_security_settings_server_gate.sql",
    );
    ...
    for (const surface of [twoStepHook, passcodeHook, twoStepPage]) {
      ...
      expect(surface).not.toMatch(
        /from\("(two_step_auth|user_passcode)"\)[\s\S]{0,180}\.(insert|upsert|update|delete)/,
      );
    }
  });
  ...
  it("keeps legacy active-session heartbeats and login alerts server-scoped", () => {
    const presenceFn = read(
      "supabase/functions/user-session-presence/index.ts",
    );
    const presenceGate = read(
      "supabase/migrations/20260601071500_user_session_presence_server_gate.sql",
    );
    ...
    expect(sessionsHook).not.toMatch(
      /from\("(user_sessions|login_alerts)"\)[\s\S]{0,220}\.(insert|update|delete|upsert)/,
    );
  });
  ...
  it("keeps Secret Chat device key writes server-scoped while preserving key reads", () => {
    ...
    const deviceKeyGate = read(
      "supabase/migrations/20260601073000_device_keys_server_gate.sql",
    );
    ...
    expect(secretChat).not.toMatch(
      /from\("device_keys"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
  });

  it("keeps privacy preference writes behind an allowlisted server update", () => {
    const privacyFn = read(
      "supabase/functions/privacy-settings-update/index.ts",
    );
    const privacyGate = read(
      "supabase/migrations/20260601074500_privacy_settings_server_gate.sql",
    );
    ...
    for (const surface of [allowMessages, sensitiveMedia, privacyPage]) {
      expect(surface).toMatch(
        /functions\.invoke\(\s*["']privacy-settings-update["']/,
      );
      expect(surface).not.toMatch(
        /from\("privacy_settings"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
      );
    }
  });

  it("keeps close-friends mutations behind verified server-side ownership checks", () => {
    const closeFriendFn = read(
      "supabase/functions/close-friend-manage/index.ts",
    );
    const closeFriendGate = read(
      "supabase/migrations/20260601080000_close_friends_server_gate.sql",
    );
    ...
    expect(closeFriendsPage).toContain(
      'functions.invoke("close-friend-manage"',
    );
    expect(closeFriendsPage).toContain('from("close_friends")');
    expect(closeFriendsPage).not.toMatch(
      /from\("close_friends"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
  });
  ...
  it("keeps user block and unblock mutations behind verified server-side ownership checks", () => {
    ...
    const blockGate = read(
      "supabase/migrations/20260601081500_blocked_users_server_gate.sql",
    );
    const surfaces = [
    ...
    for (const surface of surfaces) {
      expect(surface).toMatch(
        /functions\.invoke\(\s*["']block-user-manage["']/,
      );
      expect(surface).not.toMatch(
        /from\("blocked_users"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
      );
      expect(surface).not.toMatch(
        /dbFrom\("blocked_users"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
      );
    }
  });
  ...
  it("keeps chat thread preference mutations behind an allowlisted server update", () => {
    const settingsFn = read(
      "supabase/functions/chat-thread-settings-update/index.ts",
    );
    const settingsGate = read(
      "supabase/migrations/20260601083000_chat_thread_settings_server_gate.sql",
    );
    ...
    expect(threadSettings).toContain(
      'functions.invoke("chat-thread-settings-update"',
    );
    expect(threadSettings).toContain('from("chat_thread_settings")');
    expect(threadSettings).not.toMatch(
      /from\("chat_thread_settings"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
  });

  it("keeps muted conversation mutations behind verified server-side ownership checks", () => {
    const muteFn = read(
      "supabase/functions/muted-conversation-manage/index.ts",
    );
    const muteGate = read(
      "supabase/migrations/20260601084500_muted_conversations_server_gate.sql",
    );
    ...
    expect(mutedChatsPage).toContain(
      'functions.invoke("muted-conversation-manage"',
    );
    expect(mutedChatsPage).toContain('from("muted_conversations")');
    expect(mutedChatsPage).not.toMatch(
      /from\("muted_conversations"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
  });
  ...
  it("keeps contact-list mutations behind verified server-side ownership checks", () => {
    ...
    const contactGate = read(
      "supabase/migrations/20260601090000_user_contacts_server_gate.sql",
    );
    ...
    for (const surface of [contactsHook, contactRequestsHook]) {
      ...
      expect(surface).not.toMatch(
        /from\("user_contacts"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
      );
    }
  });

  it("keeps contact request lifecycle mutations behind verified server-side ownership checks", () => {
    const requestFn = read(
      "supabase/functions/contact-request-manage/index.ts",
    );
    const requestGate = read(
      "supabase/migrations/20260601091500_contact_requests_server_gate.sql",
    );
    const contactRequestsHook = read("src/hooks/useContactRequests.ts");
    const contactFn = read("supabase/functions/contact-manage/index.ts");
    ...
    expect(contactRequestsHook).toContain(
      'functions.invoke("contact-request-manage"',
    );
    ...
    expect(contactRequestsHook).not.toMatch(
      /from\("contact_requests"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
    expect(contactFn).toContain('from("contact_requests")');
  });

  it("keeps legacy user safety actions behind verified server-side ownership checks", () => {
    const safetyActionFn = read(
      "supabase/functions/user-safety-action-manage/index.ts",
    );
    const safetyActionGate = read(
      "supabase/migrations/20260601093000_user_safety_actions_server_gate.sql",
    );
    const mutedBlockedPage = read("src/pages/MutedBlockedUsersPage.tsx");
    const socialSafetyFn = read(
      "supabase/functions/social-safety-report/index.ts",
    );

    expect(safetyActionFn).toContain(
      'withSecurity("user-safety-action-manage"',
    );
    ...
    expect(mutedBlockedPage).toContain(
      'functions.invoke("user-safety-action-manage"',
    );
    expect(mutedBlockedPage).toContain('from("user_safety_actions")');
    expect(mutedBlockedPage).not.toMatch(
      /from\("user_safety_actions"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
  ...
  it("keeps push subscription device revocation behind verified server-side ownership checks", () => {
    ...
    const pushGate = read(
      "supabase/migrations/20260601094500_push_subscriptions_server_gate.sql",
    );
    ...
    const webUnregister = read(
      "supabase/functions/unregister-web-push/index.ts",
    );
    ...
    expect(pushDevicesPage).not.toMatch(
      /from\("push_subscriptions"\)[\s\S]{0,220}\.(insert|upsert|update|delete)/,
    );
  ...
  it("scopes user notification update and delete to the owner via RLS", () => {
    const notificationFn = read(
      "supabase/functions/notification-manage/index.ts",
    );
    const ownerPolicies = read(
      "supabase/migrations/20260403141649_0f3d28ca-54fe-450f-863a-f0e471f03d15.sql",
    );
    const gateNeutralised = read(
      "supabase/migrations/20260616090000_notifications_user_owned_mutations.sql",
    );
    ...
    const personalNotifications = read(
      "src/pages/app/personal/PersonalNotificationsPage.tsx",
    );
    const notificationManageClient = read(
      "src/lib/notifications/notificationManage.ts",
    );
    ...
    for (const policy of [
      "notifications_block_direct_update",
      "notifications_block_direct_delete",
    ]) {
    ...
    expect(notificationManageClient).toContain(
      "VITE_NOTIFICATION_MANAGE_ENABLED",
    );
    expect(notificationManageClient).toContain(
      'functions.invoke("notification-manage"',
    );
    expect(notificationManageClient).toContain(
      "NotificationManageUnavailableError",
    );
    expect(notificationManageClient).not.toMatch(
      /from\(["']notifications["']\)[\s\S]{0,260}\.(update|delete)/,
    );
    ...
    expect(
      existsSync(
        path.join(root, "src/components/rides/RideNotificationCenter.tsx"),
      ),
    ).toBe(false);
  });
  ...
  it("keeps talent invite notification creation behind verified server-side intake", () => {
    const inviteFn = read(
      "supabase/functions/talent-invite-notification/index.ts",
    );
    const inviteGate = read(
      "supabase/migrations/20260601101500_job_invite_notifications_server_gate.sql",
    );
    const findTalent = read("src/components/careers/FindTalentTab.tsx");
    const talentInviteClient = read(
      "src/lib/notifications/talentInviteNotification.ts",
    );
    ...
    expect(inviteGate).toContain(
      "notifications_block_direct_job_invite_insert",
    );
    ...
    expect(talentInviteClient).toContain(
      'functions.invoke("talent-invite-notification"',
    );
    expect(talentInviteClient).toContain(
      "VITE_TALENT_INVITE_NOTIFICATION_ENABLED",
    );
    expect(talentInviteClient).toContain(
      "TalentInviteNotificationUnavailableError",
    );
    expect(findTalent).toContain(
      "@/lib/notifications/talentInviteNotification",
    );
    expect(findTalent).not.toMatch(
      /from\("notifications"\)[\s\S]{0,260}\.insert/,
    );
  });

  it("keeps admin broadcast notification creation behind admin-only server-side intake", () => {
    const broadcastFn = read(
      "supabase/functions/admin-broadcast-notification/index.ts",
    );
    const broadcastGate = read(
      "supabase/migrations/20260601103000_admin_broadcast_notifications_server_gate.sql",
    );
    const adminBroadcast = read("src/pages/admin/AdminBroadcastPage.tsx");
    const adminBroadcastClient = read(
      "src/lib/notifications/adminBroadcastNotification.ts",
    );

    expect(broadcastFn).toContain(
      'withSecurity("admin-broadcast-notification"',
    );
    ...
    expect(broadcastGate).toContain(
      "notifications_block_direct_admin_broadcast_insert",
    );
    expect(broadcastGate).toContain("AS RESTRICTIVE");
    expect(broadcastGate).toContain("ADD COLUMN IF NOT EXISTS role text");
    expect(broadcastGate).toContain(
      "COALESCE(template, '') <> 'admin_broadcast'",
    );
    expect(broadcastGate).toContain("trusted server-side ingestion");

    expect(adminBroadcastClient).toContain(
      'functions.invoke("admin-broadcast-notification"',
    );
    expect(adminBroadcastClient).toContain(
      "VITE_ADMIN_BROADCAST_NOTIFICATION_ENABLED",
    );
    expect(adminBroadcastClient).toContain(
      "AdminBroadcastNotificationUnavailableError",
    );
    expect(adminBroadcastClient).toContain('action: "preview"');
    expect(adminBroadcastClient).toContain('action: "send"');
    expect(adminBroadcast).toContain(
      "@/lib/notifications/adminBroadcastNotification",
    );
    expect(adminBroadcast).not.toMatch(
      /from\("notifications" as any\)[\s\S]{0,260}\.insert/,
    );
  ...
  it("keeps social notification writes and read-state changes behind server-side intake", () => {
    const socialFn = read(
      "supabase/functions/social-notification-manage/index.ts",
    );
    const socialGate = read(
      "supabase/migrations/20260601110000_user_notifications_server_gate.sql",
    );
    const socialHook = read("src/hooks/useSocialNotifications.ts");
    const socialClient = read(
      "src/lib/notifications/socialNotificationManage.ts",
    );
    ...
    expect(socialClient).toContain(
      'functions.invoke("social-notification-manage"',
    );
    ...
    expect(socialHook).toContain(
      "@/lib/notifications/socialNotificationManage",
    );
    expect(socialHook).not.toMatch(
      /from\("user_notifications"\)[\s\S]{0,260}\.(insert|update|delete)/,
    );
  });
});
