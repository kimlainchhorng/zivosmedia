...
describe("payouts, earnings, and balances workflow", () => {
  ...
  it("requires authenticated owners, drivers, creators, or admins before payout changes", () => {
    ...
    expect(instant).toContain("status: 503");
    ...
    expect(eats).toContain("Payout method does not belong to this restaurant");
    ...
    expect(merchant).toContain(
      'withIdempotency(req, "merchant-payout-request", userId',
    );
    ...
    expect(merchantWallet).toMatch(
      serverGatedInvoke("merchant-payout-request"),
    );
    ...
    expect(merchantPayoutGate).toContain(
      'DROP POLICY IF EXISTS "Merchants can request payouts"',
    );
    expect(merchantPayoutGate).toContain(
      "server-side owner and balance checks",
    );

    expect(refundRequest).toContain(
    ...
    expect(refundRequestGate).toContain(
      "COALESCE(category, 'general') <> 'refund_request'",
    );
    expect(walletPage).toMatch(
      /functions\.invoke\(\s*"refund-request-submit"/,
    );
    ...
    expect(arPayout).toContain(
      'withIdempotency(req, "ar-payout-record", userId',
    );
    ...
    expect(arPayoutGate).toContain(
      'DROP POLICY IF EXISTS "Owners manage their ar_payouts"',
    );
    expect(arPayoutGate).toContain(
    ...
    expect(cafeTipPayout).toContain(
      'withIdempotency(req, "cafe-tip-payout-record", userId',
    );
    ...
    expect(cafeTips).not.toContain(
      '.from("cafe_tip_payouts" as never)\\n      .insert',
    );
    expect(cafeTips).not.toContain(
      '.from("cafe_tip_payout_lines" as never)\\n      .insert',
    );
    expect(cafeTipGate).toContain(
      "DROP POLICY IF EXISTS cafe_tip_payouts_owner_manage",
    );
    expect(cafeTipGate).toContain(
      "DROP POLICY IF EXISTS cafe_tip_payout_lines_owner_manage",
    );
    expect(cafeTipGate).toContain(
      "Cafe tip payout headers are written by cafe-tip-payout-record",
    );

    expect(salonPayout).toContain(
      'withSecurity("salon-commission-payout-record"',
    );
    expect(salonPayout).toContain("enforceAal2(authHeader, corsHeaders)");
    expect(salonPayout).toContain(
      'withIdempotency(req, "salon-commission-payout-record", userId',
    );
    ...
    expect(salonCommissions).toMatch(
      serverGatedInvoke("salon-commission-payout-record"),
    );
    expect(salonCommissions).toContain(
      '"Idempotency-Key": `salon-commission-payout-',
    );
    expect(salonCommissions).not.toContain(
      '.from("salon_commission_payouts")\\n      .insert',
    );
    expect(salonCommissions).not.toContain(
      '.from("salon_commission_payouts").delete',
    );
    expect(salonPayoutGate).toContain(
      'DROP POLICY IF EXISTS "Owners manage commission payouts - all"',
    );
    expect(salonPayoutGate).toContain(
      "Salon commission payouts are written by salon-commission-payout-record",
    );

    expect(payoutMethod).toMatch(
      /withSecurity\(\s*"customer-payout-method-record"/,
    );
    expect(payoutMethod).toContain("enforceAal2(authHeader, corsHeaders)");
    expect(payoutMethod).toMatch(
      /withIdempotency\(\s*req,\s*"customer-payout-method-record",\s*userId/,
    );
    ...
    for (const source of [walletPage, driverPayouts, lodgePayoutAccount]) {
      expect(source).toMatch(
        serverGatedInvoke("customer-payout-method-record"),
      );
    ...
    expect(payoutMethodGate).toContain(
    ...
    expect(creatorPayoutMethod).toContain(
    ...
    expect(creatorPayoutMethod).toContain(
      'withIdempotency(req, "creator-payout-method-record", userId',
    );
    ...
    expect(paypalHook).not.toContain(
      'insert({ user_id: params.userId, payout_method: "paypal"',
    );
    expect(creatorPayoutMethodGate).toContain(
    ...
    expect(creatorPayoutMethodGate).toContain(
      "NEW.payout_details IS DISTINCT FROM OLD.payout_details",
    );
    expect(creatorProfileProtectedGate).toContain(
      "creator_profile_protected_fields_server_gate_required",
    );
    expect(creatorProfileProtectedGate).toContain(
      "NEW.total_earnings_cents IS DISTINCT FROM OLD.total_earnings_cents",
    );
    expect(creatorProfileProtectedGate).toContain(
      "NEW.subscriber_count IS DISTINCT FROM OLD.subscriber_count",
    );
    expect(creatorProfileProtectedGate).toContain(
      "NEW.follower_count IS DISTINCT FROM OLD.follower_count",
    );
    expect(creatorProfileProtectedGate).toContain(
    ...
    expect(creatorProfileProtectedGate).toContain(
      "allowing normal profile edits",
    );

    expect(methodsMigration).toContain(
  ...
  it("renders Connect, payout, and earnings states for pending, active, restricted, and failed paths", () => {
    ...
    expect(unifiedCard).toContain("Wallet payouts are paused");
    expect(unifiedCard).toContain(
      "Stripe and PayPal cash out are temporarily unavailable",
    );
    expect(unifiedCard).toContain("Your wallet balance is unchanged");
  ...
  it("keeps active payout retries idempotent while every generic Wallet rail stays retired", () => {
    const retiredPaths = [
    ...
    const payoutNotifications = read(
    ...
    for (const relativePath of retiredPaths) {
      ...
      expect(text).not.toContain("stripe.payouts.create");
      expect(text).not.toContain("/v1/payments/payouts");
      expect(text).not.toContain("process_customer_wallet_withdrawal");
    }
    ...
    expect(atomicWithdrawal).toContain(
    ...
    expect(payoutNotifications).toContain(
  ...
  it("records balance movement, payout resolution, and admin decisions in auditable ledgers", () => {
    ...
    for (const text of retiredWalletPayouts) {
      expect(text).toContain(
        'code: "wallet_cashout_authority_unavailable"',
      );
      expect(text).not.toContain("customer_wallet_transactions");
      expect(text).not.toContain("process_customer_wallet_withdrawal");
      expect(text).not.toContain("balance_cents: newBalance");
    }
    expect(atomicWithdrawal).toContain(
    ...
    expect(creatorMigration).toContain(
  ...
  it("keeps share-to-earn wallet and loyalty rewards behind server-side intake", () => {
    const shareFunction = read(
      "supabase/functions/share-to-earn-manage/index.ts",
    );
    const shareGate = read(
      "supabase/migrations/20260601114500_share_to_earn_server_gate.sql",
    );
    const dataApiGrants = read(
      "supabase/migrations/20260601154700_data_api_grants_referrals_and_webhook_events.sql",
    );
    ...
    expect(dataApiGrants).toContain(
    ...
    expect(shareHook).not.toMatch(
      /from\("user_referral_codes"\)[\s\S]{0,180}\.(insert|update|delete|upsert)/,
    );
    expect(shareHook).not.toMatch(
      /from\("referral_shares"\)[\s\S]{0,180}\.(insert|update|delete|upsert)/,
    );
    expect(shareHook).not.toMatch(
      /from\("referral_conversions"\)[\s\S]{0,180}\.(insert|update|delete|upsert)/,
    );
    expect(shareHook).not.toMatch(
      /from\("customer_wallet_transactions"\)[\s\S]{0,180}\.(insert|update|delete|upsert)/,
    );
    expect(shareHook).not.toMatch(
      /from\("loyalty_points"\)[\s\S]{0,180}\.(insert|update|delete|upsert)/,
    );
  });

  it("keeps loyalty balance mutations behind server-side point accounting", () => {
    const loyaltyFunction = read(
      "supabase/functions/loyalty-points-manage/index.ts",
    );
    const loyaltyGate = read(
      "supabase/migrations/20260601120000_loyalty_points_server_gate.sql",
    );
    ...
    expect(loyaltyGate).toContain(
      'DROP POLICY IF EXISTS "Users can insert own loyalty points"',
    );
    expect(loyaltyGate).toContain(
      'DROP POLICY IF EXISTS "Users can update own loyalty points"',
    );
    ...
    expect(loyaltyHook).not.toMatch(
      /from\("loyalty_points"\)[\s\S]{0,240}\.(insert|update|delete|upsert)/,
    );
  });

  it("keeps wallet checkout debits behind atomic server-side balance checks", () => {
    const walletFunction = read(
      "supabase/functions/wallet-payment-deduct/index.ts",
    );
    const walletGate = read(
      "supabase/migrations/20260601121500_customer_wallet_payment_server_gate.sql",
    );
    ...
    expect(walletHook).not.toMatch(
      /from\("customer_wallets"\)[\s\S]{0,240}\.(insert|update|delete|upsert)/,
    );
    expect(walletHook).not.toMatch(
      /from\("customer_wallet_transactions"\)[\s\S]{0,240}\.(insert|update|delete|upsert)/,
    );
  });
});
