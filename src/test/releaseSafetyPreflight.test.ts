...
describe("release safety preflight contracts", () => {
  ...
  it("keeps Supabase secret misuse regressions in the release-safety lane", () => {
    ...
    expect(source("docs/production-preflight-report.md")).toMatch(/- Edge Function browser gates: gatedFunctions=6, failures=\d+/);
    ...
    expect(packageJson).toContain("npm run check:business-identity");
    expect(packageJson).toContain("npm run deploy:preflight:check-production-summary");
