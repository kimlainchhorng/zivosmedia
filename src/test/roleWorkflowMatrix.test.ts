...
describe("role workflow matrix", () => {
  ...
  it("keeps driver money workflows gated and creator money workflows disabled", () => {
    ...
    const adminDriverPayouts = source(
      "src/pages/admin/AdminDriverPayoutsPage.tsx",
    );
    const resolveDriverPayout = source(
    ...
    expect(creatorMigration).toContain(
      "REVOKE EXECUTE ON FUNCTION public.request_live_earnings_payout",
    );
    ...
    expect(adminDriverPayouts).toMatch(
      /invokeSensitive(?:<[^>]+>)?\(\s*"resolve-driver-earning-payout"/,
    );
    expect(resolveDriverPayout).toContain(
      'withSecurity("resolve-driver-earning-payout"',
    );
    expect(resolveDriverPayout).toContain(
    ...
    expect(resolveDriverPayout).toContain(
      'admin.from("admin_driver_actions").insert',
    );
  });
  ...
  it("keeps protected-route exceptions and database policies aligned with role boundaries", () => {
    ...
    const roleGrants = source(
      "supabase/migrations/20260531184042_role_workflow_data_api_grants.sql",
    );
    const employeeGate = source(
      "supabase/migrations/20260601141500_store_employees_server_gate.sql",
    );
    const shiftGate = source(
      "supabase/migrations/20260601143000_employee_shifts_server_gate.sql",
    );
    const ruleGate = source(
      "supabase/migrations/20260601144500_employee_rules_server_gate.sql",
    );
    const shoppingPolicy = source(
      "supabase/migrations/20260531194500_shopping_orders_customer_driver_policy.sql",
    );
    const driverRls = source(
    ...
    expect(app).toMatch(
      /path="\/admin\/stores\/:storeId"[\s\S]{0,180}<ProtectedRoute[\s\S]{0,100}requireAdmin=\{true\}[\s\S]{0,100}allowStoreOwner=\{true\}/,
    );
    for (const route of ["support", "user-accounts"]) {
      expect(app).toMatch(
        new RegExp(
          `path="/admin/${route}"[\\s\\S]{0,180}<ProtectedRoute[\\s\\S]{0,100}requireAdmin=\\{true\\}[\\s\\S]{0,100}allowSupport=\\{true\\}`,
        ),
      );
    }
    ...
    expect(shoppingPolicy).toContain(
  ...
  it("tracks cross-vertical route coverage in role readiness", () => {
    ...
    expect(matrix).toContain(
      "Keep customer booking, shop owner, staff, driver, support, and admin workflows green while creator monetization remains retired.",
    );
    expect(staffDriverCreator).toContain("retired creator monetization");
    expect(merchantPayoutOwnerOps).toContain(
      "merchant payout owner ops access",
    );
