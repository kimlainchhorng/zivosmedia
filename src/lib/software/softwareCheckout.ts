...
function friendlyMessage(
  ...
  if (
    normalized.includes("existing software subscription") ||
  ...
  if (normalized.includes("owner access"))
    return "Only a business owner or administrator can manage this subscription.";
  if (normalized.includes("pricing plan") || normalized.includes("plan_id"))
    return "That plan is not currently available.";
  if (normalized.includes("stripe") && normalized.includes("configured"))
...
async function functionError(
  ...
  let code =
    error instanceof Error ? error.message : String(error || "checkout_error");
  try {
    const context = (error as { context?: { json?: () => Promise<unknown> } })
      ?.context;
    const payload = context?.json
      ? ((await context.json()) as { error?: unknown; message?: unknown })
      : null;
...
export function newSoftwareBillingIdempotencyKey(
...
function dedicatedSoftwareBillingHost(): boolean {
  return (
    ...
    isZivoSoftwareHost(window.location.hostname)
  );
}
...
export async function createSoftwareCheckoutUrl(input: {
  ...
  return {
    url,
    checkoutSessionId: String(
      payload.checkout_session_id || payload.session_id || "",
    ),
  ...
  const { data, error } = await supabase.functions.invoke(
      ...
      : "zivopay-create-billing-portal",
    {
      body: {
        ...
        return_url: dedicated
          ? dedicatedReturnPath(input.returnUrl)
          : input.returnUrl,
      },
      headers: { "Idempotency-Key": input.idempotencyKey },
    },
