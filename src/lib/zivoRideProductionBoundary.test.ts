...
describe("ZIVO Ride production boundary", () => {
  it("accepts only the canonical production Ride origin", () => {
    expect(resolveRideAppBaseUrl(ZIVO_RIDE_PRODUCTION_ORIGIN)?.origin).toBe(
      ZIVO_RIDE_PRODUCTION_ORIGIN,
    );
  });
  ...
  it("rejects remote HTTP, URL credentials, custom ports, queries, and fragments", () => {
    ...
    expect(
      resolveRideAppBaseUrl("https://user:pass@ride.zivosmedia.com"),
    ).toBeNull();
    expect(
      resolveRideAppBaseUrl("https://ride.zivosmedia.com:8443"),
    ).toBeNull();
    expect(
      resolveRideAppBaseUrl("https://ride.zivosmedia.com?redirect=evil"),
    ).toBeNull();
    expect(
      resolveRideAppBaseUrl("https://ride.zivosmedia.com/#evil"),
    ).toBeNull();
  });

  it("allows only the exact local Ride origin in development", () => {
    const dev = { allowLocalDevelopment: true };
    expect(resolveRideAppBaseUrl(undefined, dev)?.origin).toBe(
      "http://localhost:5177",
    );
    expect(resolveRideAppBaseUrl("http://127.0.0.1:5177", dev)?.origin).toBe(
      "http://127.0.0.1:5177",
    );
  ...
  it("maps only exact local Zivosmedia parents to their matching Ride dev origin", () => {
    const dev = { allowLocalDevelopment: true };

    expect(
      resolveLocalRideAppBaseUrl("http://localhost:8081", dev)?.origin,
    ).toBe("http://localhost:5177");
    expect(
      resolveLocalRideAppBaseUrl("http://127.0.0.1:8081", dev)?.origin,
    ).toBe("http://127.0.0.1:5177");
    expect(resolveLocalRideAppBaseUrl("http://localhost:8081")).toBeNull();
    expect(
      resolveLocalRideAppBaseUrl("https://zivosmedia.com", dev),
    ).toBeNull();
    expect(
      resolveLocalRideAppBaseUrl("http://user:pass@localhost:8081", dev),
    ).toBeNull();
    expect(resolveLocalRideAppBaseUrl("https://evil.example", dev)).toBeNull();
  });

  it("admits only parent and Ride origin pairs supported by frame-ancestors", () => {
    const dev = { allowLocalDevelopment: true };

    expect(
      canEmbedRideApp("https://zivosmedia.com", ZIVO_RIDE_PRODUCTION_ORIGIN),
    ).toBe(true);
    expect(
      canEmbedRideApp(
        "https://www.zivosmedia.com",
        `${ZIVO_RIDE_PRODUCTION_ORIGIN}/history`,
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      canEmbedRideApp("http://localhost:8081", "http://127.0.0.1:5177", dev),
    ).toBe(true);
    expect(
      canEmbedRideApp("http://127.0.0.1:8081", "http://127.0.0.1:5177", dev),
    ).toBe(true);
    expect(
      canEmbedRideApp("http://localhost:8081", "http://localhost:5177"),
    ).toBe(false);
    expect(
      canEmbedRideApp("https://evil.example", ZIVO_RIDE_PRODUCTION_ORIGIN),
    ).toBe(false);
    expect(
  ...
  it("accepts only the central production authorize endpoint", () => {
    ...
    expect(
      getRideAuthorizeUrl({
        ...
        url: message.url.replace(
          "zivosmedia.com/auth",
          "zivosmedia.com.evil.example/auth",
        ),
      }),
    ).toBeNull();
    expect(
    ...
    expect(
      getRideAuthorizeUrl({
        ...
        url: message.url.replace(
          ...
          encodeURIComponent(
            "https://zivosmedia.com/auth/callback?source=zivosmedia",
          ),
        ),
      }),
    ).toBeNull();
  ...
  it("keeps the local authorize endpoint development-only", () => {
    ...
    expect(
      getRideAuthorizeUrl(message, { allowLocalDevelopment: true })?.origin,
  ...
  it("rejects the retired local hub port so local Ride always targets the same zivosmedia app", () => {
    ...
    expect(
      getRideAuthorizeUrl(
        { type: "zivo-ride:authorize", url: url.toString() },
