...
- **Client-side write gate** (`src/lib/security/contentLinkValidation.ts`):
  `confirmContentSafe(text, label)` runs `assessLinkSync` (`src/hooks/useLinkRisk.ts` + `src/lib/urlSafety.ts`) on every URL extracted from the input. Hard-blocks: ZIVO typosquats (Levenshtein ≤ 2 from `zivosmedia.com` / `myzivo.lovable.app`), embedded credentials, dangerous protocols, impersonation. Surfaces as suspicious: 23 URL shorteners, suspicious TLDs (`.zip`, `.tk`, etc.), punycode/IDN, raw IPs, oversized URLs.
- **Client-side render gate** (`src/components/social/SafeCaption.tsx`): all user-content surfaces (including FeedPage, ReelsFeedPage, Profile, PublicProfile, ChatContact, Communities, Stores, Hotels, Reviews, etc.) tokenize URLs through SafeCaption, rendering blocked links struck through and non-clickable, suspicious links with a Caution badge + interstitial.
...
- **Contact**: security@zivosmedia.com
...
If you discover a security vulnerability, please report it to **security@zivosmedia.com**. Do not create public issues. We will acknowledge receipt within 48 hours and provide a timeline for resolution.
